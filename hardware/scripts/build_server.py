...
Accepts POST /build with JSON body:
  { "env": "esp32_wifi", "fsd": 1, "vehicle": 1, "body": 0 }

Bus flags (0=off, 1=on):
  fsd     — FSD / Autopilot CAN (X179 pins 13-14), default 1
...
def build_firmware(env, fsd=1, vehicle=0, body=0):
    """Run PlatformIO build and return (firmware_bytes, extension) or raise."""
    if env not in VALID_ENVS:
        raise ValueError(f"Unknown env: {env}")
    for name, val in [("fsd", fsd), ("vehicle", vehicle), ("body", body)]:
        if val not in (0, 1):
            raise ValueError(f"Invalid {name} value: {val}")

    bus_flags = f"-DBUS_FSD_ACTIVE={fsd} -DBUS_VEHICLE_ACTIVE={vehicle} -DBUS_BODY_ACTIVE={body}"
...
class BuildHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not isinstance(env, str) or not ENV_RE.match(env):
            ...
            return
        for name, val in [("fsd", fsd), ("vehicle", vehicle), ("body", body_bus)]:
            if not isinstance(val, int) or val not in (0, 1):
                self._json_response(400, {"error": f"Invalid {name} value"})
                return

        try:
            fw_bytes, ext = build_firmware(env, fsd, vehicle, body_bus)
