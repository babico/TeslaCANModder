...
class FakeSerial {
public:
  void print(const char* s) { capturedOutput += s; }
  void print(long n) {
    char buf[32];
...
bool executeStreamCmd(const char* cmd, State& s) {
  if (strcmp(cmd, "stream:on") == 0) { s.streamEnabled = true; return true; }
  if (strcmp(cmd, "stream:off") == 0) { s.streamEnabled = false; return true; }
  return false;
}

bool executeCanRawCmd(const char* cmd, State& s) {
  if (strcmp(cmd, "can:raw:on") == 0) { s.rawCanListen = true; return true; }
  if (strcmp(cmd, "can:raw:off") == 0) { s.rawCanListen = false; return true; }
  return false;
}

bool executeFsdCmd(const char* cmd, State& s) {
  if (strcmp(cmd, "fsd:on") == 0) { s.fsdEnabled = true; return true; }
  if (strcmp(cmd, "fsd:off") == 0) { s.fsdEnabled = false; return true; }
  if (strcmp(cmd, "fsd:toggle") == 0) { s.fsdEnabled = !s.fsdEnabled; return true; }
  return false;
}

bool executeNagCmd(const char* cmd, State& s) {
  if (strcmp(cmd, "nag:on") == 0) { s.nagSuppress = true; return true; }
  if (strcmp(cmd, "nag:off") == 0) { s.nagSuppress = false; return true; }
  return false;
}
...
bool executeIsaChimeCmd(const char* cmd, State& s) {
  if (strcmp(cmd, "isa-chime:off") == 0) { s.isaChimeSuppress = true; return true; }
  if (strcmp(cmd, "isa-chime:on") == 0) { s.isaChimeSuppress = false; return true; }
  return false;
}
...
bool execWindowCmd(const char*, State&) { return false; }
bool execSentryCmd(const char*, State&) { return false; }
bool execClimateCmd(const char*, State&) { return false; }
bool execChargeCmd(const char*, State&) { return false; }
bool execDriveCmd(const char*, State&) { return false; }
bool executeVehicleCmd(const char*, State&) { return false; }

// ── Now define the serial functions we're testing ───────────────────────────
// (Inline replicas from serial/esp32.h, adapted for test env)

void printStr(const char* s) { Serial.print(s); }
void printNum(long n) { Serial.print(n); }
...
void executeCommand(const char* cmd, State& s, unsigned long now) {
  ...
  if (executeStreamCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeCanRawCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeFsdCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeNagCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeProfileCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeOffsetCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeIsaChimeCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeSummonCmd(cmd, s)) { sendAck(cmd); return; }
  if (executeVariantCmd(cmd, s)) { sendAck(cmd); return; }
...
void test_cmd_fsd_toggle() {
  State s = makeState();
  s.fsdEnabled = false;
  executeCommand("fsd:toggle", s, 1000);
  TEST_ASSERT_TRUE(s.fsdEnabled);
}
