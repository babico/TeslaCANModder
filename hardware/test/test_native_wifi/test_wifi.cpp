...
static std::string buildStateJson(State& s) {
  ...
  json += ",\"canOnline\":" + std::to_string(s.canOnline ? 1 : 0);
  ...
  json += ",\"speedOffset\":" + std::to_string(feat.speedOffset ? 1 : 0);
  ...
  json += ",\"busFsd\":" + std::to_string(BUS_FSD_ACTIVE);
...
void test_wifi_json_contains_hardware_block() {
  ...
  TEST_ASSERT_TRUE(json.find("\"busFsd\":1") != std::string::npos);
...
void test_wifi_json_hw4_features() {
  ...
  TEST_ASSERT_TRUE(json.find("\"speedOffset\":0") != std::string::npos);
}

void test_wifi_json_hw3_features() {
  State s = makeState(HW3);
  std::string json = buildStateJson(s);
  TEST_ASSERT_TRUE(json.find("\"speedOffset\":1") != std::string::npos);
  TEST_ASSERT_TRUE(json.find("\"summon\":1") != std::string::npos);
...
void test_wifi_json_all_state_fields() {
  ...
  TEST_ASSERT_TRUE(json.find("\"canOnline\":1") != std::string::npos);
