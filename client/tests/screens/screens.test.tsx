...
jest.mock("../../src/components/docs/MarkdownRenderer", () => ({
	MarkdownRenderer: ({ markdown }: { markdown: string }) =>
		React.createElement("MarkdownRenderer", { markdown }),
}));
...
describe("Screen UI coverage", () => {
	...
	it("renders ControlsScreen and opens command palette", () => {
		...
		expect(
			renderer.root.findAll((node) => (node.type as any) === "Modal").length,
		).toBeGreaterThan(0);
	});
			...
			renderer = TestRenderer.create(
				React.createElement(ConsoleScreen, {
					...
					selectedDecoderDataset: {
						...
						label: "Known",
						dataset: {
							dataset_source: { vehicle: "Tesla", firmware: "", mcu: "", soc: "" },
							frames: [],
						},
					},
	...
	it("renders DocsScreen and navigates docs tree", () => {
		...
		expect(
			renderer.root.findAll((node) => (node.type as any) === "MarkdownRenderer").length,
		).toBe(1);
	});
});
