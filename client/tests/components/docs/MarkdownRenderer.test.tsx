...
jest.mock("react-native-markdown-display", () => {
  return ({
    ...
    return React.createElement("MarkdownDisplayMock", null, children);
  };
});
...
describe("MarkdownRenderer", () => {
  it("renders markdown through package-based renderer", () => {
    ...
    TestRenderer.act(() => {
      renderer = TestRenderer.create(
        React.createElement(MarkdownRenderer, { markdown: "# Title\n\n- [x] Task" })
      );
    });

    const nodes = renderer.root.findAll((node) => (node.type as any) === "MarkdownDisplayMock");
  ...
  it("forwards onLinkPress handler", () => {
    ...
    const result = markdownPropsCapture.onLinkPress?.("../../../src/components/docs/getting-started.md");
