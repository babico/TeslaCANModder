...
describe("INGEST_FRAMES", () => {
	...
	it("caps buffer at maxBuffer and tracks evictions", () => {
		const cap = 20;
		const s = dispatch(initialFrameIngestionState({ maxBuffer: cap }), {
			type: "INGEST_FRAMES",
			payload: { frames: frames(30) },
		});
...
describe("SET_CONFIG", () => {
	it("updates config fields", () => {
		const s = dispatch(initialFrameIngestionState(), {
			type: "SET_CONFIG",
			payload: { windowSize: 100, busFilter: "body" },
		});
	...
	it("clamps windowSize to [1, 200]", () => {
		const s1 = dispatch(initialFrameIngestionState(), {
			type: "SET_CONFIG",
			payload: { windowSize: 999 },
		});
		expect(s1.config.windowSize).toBe(200);

		const s2 = dispatch(initialFrameIngestionState(), {
			type: "SET_CONFIG",
	...
	it("clamps sampleStep to [1, 20]", () => {
		const s1 = dispatch(initialFrameIngestionState(), {
			type: "SET_CONFIG",
			payload: { sampleStep: 100 },
		});
		expect(s1.config.sampleStep).toBe(20);

		const s2 = dispatch(initialFrameIngestionState(), {
			type: "SET_CONFIG",
			payload: { sampleStep: 0 },
		});
...
describe("PAUSE / RESUME", () => {
	...
	it("RESUME sets paused=false", () => {
		const s = dispatch(initialFrameIngestionState({ paused: true }), { type: "RESUME" });
...
describe("selectVisibleFrames", () => {
	it("returns empty array when paused", () => {
		const s = dispatch(initialFrameIngestionState({ paused: true }), {
			type: "INGEST_FRAMES",
			payload: { frames: frames(5) },
		});
		expect(selectVisibleFrames(s)).toHaveLength(0);
	});

	it("applies windowSize", () => {
		const s = dispatch(initialFrameIngestionState({ windowSize: 3 }), {
			type: "INGEST_FRAMES",
			payload: { frames: frames(10) },
		});
		expect(selectVisibleFrames(s)).toHaveLength(3);
	});

	it("applies bus filter", () => {
		const mixed = [frame("v1", "vehicle"), frame("b1", "body"), frame("v2", "vehicle")];
		const s = dispatch(initialFrameIngestionState({ busFilter: "body" }), {
			type: "INGEST_FRAMES",
			payload: { frames: mixed },
		});
	...
	it("applies text filter on id, name, data", () => {
		...
		const s = dispatch(initialFrameIngestionState({ textFilter: "speed" }), {
			type: "INGEST_FRAMES",
			payload: { frames: mixed },
		});
		const visible = selectVisibleFrames(s);
	...
	it("applies sample step (every other frame)", () => {
		const s = dispatch(initialFrameIngestionState({ sampleStep: 2, windowSize: 10 }), {
			type: "INGEST_FRAMES",
			payload: { frames: frames(10) },
		});
		const visible = selectVisibleFrames(s);
...
describe("selectIngestionStats", () => {
	it("reports correct buffer utilization", () => {
		const s = dispatch(initialFrameIngestionState({ maxBuffer: 100 }), {
			type: "INGEST_FRAMES",
			payload: { frames: frames(40) },
		});
