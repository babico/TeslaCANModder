...
describe("COMMAND_PENDING", () => {
  it("adds entry with status=pending and increments totalDispatched", () => {
    ...
    expect(s.entries[0]).toMatchObject({ id: "1", command: "fsdOn", status: "pending", startedAt: T });
...
describe("COMMAND_ACKED", () => {
  ...
  it("updates command string if provided", () => {
    const s = dispatch(
      ...
      { type: "COMMAND_ACKED", payload: { id: "1", command: "built_cmd", finishedAt: T + 50 } }
    );
  ...
  it("ignores ack for unknown id (no error, no state mutation)", () => {
    const s = dispatch(
      initialCommandBusState(),
      { type: "COMMAND_ACKED", payload: { id: "unknown", finishedAt: T } }
    );
    expect(s.entries).toHaveLength(0);
...
describe("COMMAND_FAILED", () => {
  it("transitions pending entry to failed", () => {
    const s = dispatch(
      initialCommandBusState(),
      { type: "COMMAND_PENDING", payload: { id: "1", command: "fsdOn", startedAt: T } },
      { type: "COMMAND_FAILED", payload: { id: "1", finishedAt: T + 200, detail: "timeout" } }
    );
...
describe("PURGE_SETTLED", () => {
  ...
  it("with olderThanMs — keeps recently settled entries", () => {
    ...
    const s1 = dispatch(
      initialCommandBusState(),
      { type: "COMMAND_PENDING", payload: { id: "recent", command: "a", startedAt: now - 100 } },
      { type: "COMMAND_PENDING", payload: { id: "old", command: "b", startedAt: now - 60_000 } },
