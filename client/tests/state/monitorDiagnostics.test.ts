...
describe("monitorDiagnostics", () => {
	it("builds sorted diagnostics events from lifecycle, history, board, and snapshots", () => {
		...
		expect(events.map((entry) => entry.ts)).toEqual([1000, 999, 100, 95, 90, 85]);
	});
	...
	it("filters diagnostics by category and query", () => {
		...
		expect(filterDiagnosticsEvents({ events, category: "board", query: "" })).toEqual([
			events[1],
		]);

		expect(filterDiagnosticsEvents({ events, category: "all", query: "lock" })).toEqual([
			events[0],
		]);

		expect(filterDiagnosticsEvents({ events, category: "all", query: "error" })).toEqual([
			events[1],
		]);
	});
