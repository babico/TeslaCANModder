...
import { CONNECTION_PRESETS, useBoardConnection } from "../state/BoardConnectionContext";
...
function formatTransportLabel(type: MonitorTransportType): string {
	return (
		DISPLAY_TRANSPORT_OPTIONS.find((option) => option.id === normalizeTransportType(type))
...
export function ConnectionHeader() {
	...
	const statusLabel = conn.connectionBusy ? "Connecting" : isReady ? "Connected" : "Disconnected";
				...
				<ScrollView contentContainerStyle={styles.sheetContent}>
					...
					{visualTransportType === "serial" ? (
						<View style={styles.helperCard}>
							...
							<Text style={styles.helperBody}>
								Uses the browser serial picker. This covers direct USB serial and
								Bluetooth COM ports exposed by the OS.
							</Text>
						</View>
					) : null}
					...
					{visualTransportType === "ble" ? (
						<View style={styles.helperCard}>
							...
							<Text style={styles.helperBody}>
								Uses Web Bluetooth and the Nordic UART Service UUIDs documented in
								the BLE adapter reference.
							</Text>
						</View>
					) : null}
					...
					<View style={styles.actionRow}>
						<Pressable
							style={[
								styles.connectBtn,
								conn.connectionBusy ? styles.buttonDisabled : undefined,
							]}
					...
					<Text style={styles.lastResult} numberOfLines={3}>
						{conn.lastResult}
					</Text>
				</ScrollView>
			</Sheet>
