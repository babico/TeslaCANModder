...
function formatBoardMessageText(text: string): string {
  ...
  const formatted = lines.map((line) => {
    ...
    if (prefixedMatch) {
      ...
      return pretty
        .split("\n")
        .map((prettyLine, index) => (index === 0 ? `${prefix} ${prettyLine}` : `  ${prettyLine}`))
        .join("\n");
    }
      ...
      <View style={[styles.card, isWide ? styles.cardWide : undefined]}>
          ...
          <View style={styles.chipRow}>
            {props.decoderDatasets.map((entry) => (
              <Pressable
                ...
                style={[styles.chip, props.selectedDecoderDataset.id === entry.id ? styles.chipActive : undefined]}
              >
          ...
          <View style={styles.chipRow}>
            {BUS_FILTER_OPTIONS.map((opt) => (
              <Pressable
                ...
                style={[styles.chip, props.busFilter === opt.value ? styles.chipActive : undefined]}
              >
        ...
        <View style={styles.colNarrow}>
          ...
          <View style={styles.chipRow}>
            {WINDOW_PRESETS.map((preset) => (
              <Pressable
                ...
                style={[styles.chip, props.frameWindowSize === preset ? styles.chipActive : undefined]}
              >
          ...
          <View style={styles.chipRow}>
            {SAMPLE_PRESETS.map((preset) => (
              <Pressable
                ...
                style={[styles.chip, props.frameSampleStep === preset ? styles.chipActive : undefined]}
              >
          ...
          <View style={styles.actionRow}>
            <ActionButton title={props.frameFeedPaused ? "Paused" : "Live"} onPress={() => props.onFrameFeedPausedChange(!props.frameFeedPaused)} />
            <ActionButton title="Snapshot" onPress={props.onSaveSnapshot} variant="secondary" />
            <ActionButton title="Clear" onPress={props.onClearFeed} variant="secondary" />
          </View>
        </View>
      </View>

      <View style={styles.card}>
        <Text style={styles.label}>Live Feed ({props.visibleFrames.length}/{props.boardState.frameCount})</Text>
        <View style={styles.feedList}>
          {props.visibleFrames.slice(0, 32).map((frame) => (
            <View key={frame.key} style={styles.feedRow}>
              <Text style={styles.feedKey}>{frame.busName} 0x{frame.id.toString(16).toUpperCase()}</Text>
              <Text style={styles.feedData} numberOfLines={1}>{props.frameDecodedNameByKey[frame.key] || frame.data}</Text>
            </View>
          ))}
          {props.visibleFrames.length === 0 ? <Text style={styles.empty}>No visible frames.</Text> : null}
        </View>
      </View>
      ...
      <View style={styles.card}>
        <View style={styles.boardInfoHeaderRow}>
          ...
          <ActionButton
            ...
            onPress={() => props.onBoardInfoFeedPausedChange(!props.boardInfoFeedPaused)}
            variant="secondary"
          />
        </View>
        ...
        <View style={styles.consoleRow}>
          ...
          />
          <ActionButton title={rawBusy ? "Sending" : "Send"} onPress={() => void runRawFromConsole()} disabled={rawBusy} />
        </View>
        ...
        <ScrollView style={styles.messageList} contentContainerStyle={styles.messageListContent}>
          {boardMessages.length === 0 ? (
            ...
            boardMessages.map((message, index) => (
              <View key={`${message.id}-${message.ts}-${index}`} style={styles.messageRow}>
                <Text style={styles.messageMeta}>{message.ts} · {message.type.toUpperCase()}</Text>
                <ScrollView
                ...
                >
                  <Text style={styles.messageText}>{formatBoardMessageText(message.text)}</Text>
                </ScrollView>
              </View>
            ))
          )}
