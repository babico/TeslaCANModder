	...
	return (
		<View style={[styles.busBar, { borderColor: theme.railBorder }]}>
			...
			<View style={styles.heroHeader}>
				...
				<Pressable
					style={[styles.paletteButton, { borderColor: theme.railAccent }]}
					onPress={onOpenPalette}
				>
					<Text style={[styles.paletteButtonText, { color: theme.railAccent }]}>
						Palette
					</Text>
				</Pressable>
			</View>
	...
	return (
		<View
			style={[
				...
				{ borderColor: theme.railBorder, backgroundColor: theme.panelAccent },
			]}
		>
						...
						{column.map((cmd) => {
							...
							return (
								<CommandGridCell
									...
									onPress={() =>
										onRunCommand(cmd, gate.available ? null : gate.reason)
									}
								/>
							);
						})}
		...
		>
			...
			<View style={styles.tuningSection}>
				<View style={styles.tuningHeaderRow}>
					...
					<Text style={styles.tuningValue}>
						{profileName} · {boardState.profilePinned ? "Pinned" : "Auto"}
					</Text>
				</View>
				<View style={styles.tuningButtonWrap}>
					...
					<Pressable
						onPress={onSetProfileAuto}
						style={({ pressed }) => [
							styles.tuningButtonAuto,
							{ borderColor: theme.actionAccent },
							pressed ? styles.tuningButtonPressed : undefined,
						]}
					>
						<Text style={[styles.tuningButtonText, { color: theme.actionAccent }]}>
							Auto
						</Text>
					</Pressable>
				</View>
			...
			<View style={styles.tuningSection}>
				...
				<View style={styles.tuningTrack}>
					<View
						style={[
							styles.tuningFill,
							{ width: offsetFill, backgroundColor: theme.railAccent },
						]}
					/>
				</View>
				<View style={styles.offsetStepRow}>
					<Pressable
						onPress={() => onSetOffset(Math.max(0, offsetValue - 5))}
						style={({ pressed }) => [
							styles.offsetStepButton,
							pressed ? styles.tuningButtonPressed : undefined,
						]}
					>
					...
					<Pressable
						onPress={() => onSetOffset(Math.min(100, offsetValue + 5))}
						style={({ pressed }) => [
							styles.offsetStepButton,
							pressed ? styles.tuningButtonPressed : undefined,
						]}
					...
					>
						<Text style={[styles.offsetStepText, { color: theme.actionAccent }]}>
							Auto
						</Text>
					</Pressable>
				</View>
...
export function CommandGroupCard({ group, boardState, preset, onRun }: CommandGroupCardProps) {
	...
	return (
		<View
			style={[
				styles.card,
				!busOnline ? styles.cardOffline : undefined,
				{ borderColor: theme.railBorder, backgroundColor: theme.panelAccent },
			]}
		>
			<View style={styles.cardHeader}>
				...
				<View
					style={[
						styles.busBadge,
						{ borderColor: busOnline ? group.busColor : colors.statusDisconnected },
					]}
				>
					<Text
						style={[
							styles.busBadgeText,
							{ color: busOnline ? group.busColor : colors.statusDisconnected },
						]}
					>
						{group.busLabel}
					</Text>
			...
			{!busOnline && group.busField ? (
				<View style={styles.busOfflineBanner}>
					<Text style={styles.busOfflineText}>
						⊘ {group.busLabel} bus offline — all commands unavailable
					</Text>
				</View>
			) : null}
				...
				>
					<View style={styles.paletteHeader}>
						...
						<Pressable onPress={onRequestClose}>
							<Text style={[styles.paletteClose, { color: theme.actionAccent }]}>
								Close
							</Text>
						</Pressable>
					</View>
					...
					>
						{entries.map((entry) => (
							...
							>
								...
								>
									<Text style={styles.paletteCommandLabel}>
										{entry.item.label}
									</Text>
									<Text style={styles.paletteCommandMeta}>
										/{entry.item.name}
									</Text>
								</Pressable>
								<View style={styles.paletteActionArea}>
									<Pressable
										...
										style={[
											styles.palettePinButton,
											{ borderColor: theme.actionAccent },
										]}
									>
										<Text
											style={[
												styles.palettePinText,
												{ color: theme.actionAccent },
											]}
										>
	...
	return (
		<Pressable
			style={({ pressed }) => [
				...
				{
					backgroundColor: capsuleBackground ?? theme.capsuleBg,
					borderColor: capsuleBorder ?? theme.capsuleBorder,
				},
		...
		>
			<View style={styles.boxCellHeader}>
				<Text
					style={[styles.boxCellLabel, !available ? styles.chipTextGated : undefined]}
					numberOfLines={1}
				>
					{label}
				</Text>
				{!available ? (
					<Text style={[styles.boxCellBlockedMarker, { color: theme.warningAccent }]}>
						⊘
					</Text>
				) : null}
			</View>
			<View style={styles.boxCellFooter}>
				...
				<Text
					style={[
						styles.boxCellAction,
						{ color: available ? theme.actionAccent : colors.dashSecondary },
					]}
				>
					{action}
				</Text>
	...
	switch (type) {
		...
		case "access":
			return (
				<Svg width={18} height={18} viewBox="0 0 18 18">
					...
					/>
					<Path
						d="M6.5 8V6.5A2.5 2.5 0 0 1 9 4a2.5 2.5 0 0 1 2.5 2.5V8"
						stroke={accent}
						strokeWidth={1.4}
						fill="none"
					/>
		...
		case "climate":
			return (
				<Svg width={18} height={18} viewBox="0 0 18 18">
					<Path
						d="M9 3v12M3 9h12M4.5 4.5l9 9M13.5 4.5l-9 9"
						stroke={accent}
						strokeWidth={1.2}
						strokeLinecap="round"
					/>
		...
		case "lights":
			return (
				<Svg width={18} height={18} viewBox="0 0 18 18">
					<Path
						d="M4 9a4 4 0 0 1 8 0 4 4 0 0 1-8 0Z"
						stroke={accent}
						strokeWidth={1.4}
						fill="none"
					/>
					<Path
						d="M13 6l2-2M14 9h2M13 12l2 2"
						stroke={accent}
						strokeWidth={1.2}
						strokeLinecap="round"
					/>
		...
		case "advanced":
			return (
				<Svg width={18} height={18} viewBox="0 0 18 18">
					...
					<Path
						d="M9 2.5v2.1M9 13.4v2.1M2.5 9h2.1M13.4 9h2.1M4.4 4.4l1.5 1.5M12.1 12.1l1.5 1.5M13.6 4.4l-1.5 1.5M5.9 12.1l-1.5 1.5"
						stroke={accent}
						strokeWidth={1.1}
						strokeLinecap="round"
					/>
