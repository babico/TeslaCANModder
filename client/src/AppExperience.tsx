...
const DocsScreen = lazy(() => import("./screens/DocsScreen").then((m) => ({ default: m.DocsScreen })));
const FlasherScreen = lazy(() => import("./screens/FlasherScreen").then((m) => ({ default: m.FlasherScreen })));
...
const LEGACY_DECODED_DATASET_SOURCES: Array<{ id: string; label: string; payload: LegacyDecodedPayload }> = [
  {
    id: "legacy-mcu2",
...
const DIAGNOSTICS_CATEGORIES: DiagnosticsCategory[] = ["all", "command", "board", "snapshot", "system"];
...
function coerceLegacyDecoderDataset(
  ...
  const frames = payload.frames
    ...
    .map((frame) => ({
      ...
      frame_name: frame.frame_name ?? `Frame_${Number(frame.address_dec).toString(16).toUpperCase()}`,
      signals: Array.isArray(frame.signals)
        ? frame.signals.map((signal) => ({
            signal_name: signal.signal_name ?? "UnknownSignal",
            possible_values: Array.isArray(signal.possible_values) ? signal.possible_values : [],
          }))
        : [],
    }));
...
function matchesSelectedTransport(
...
function verifyExportIntegrity(rows: ExportRow[], expectedChecksum: number, expectedCount: number): boolean {
...
export default function AppExperience() {
  ...
  const [diagnosticsArchive, setDiagnosticsArchive] = useState<PersistedDiagnosticsArchiveEntry[]>([]);
  ...
  const legacyDecoderDatasets = useMemo(
    () =>
      LEGACY_DECODED_DATASET_SOURCES
        .map((source) => {
  ...
  const allDecoderDatasets = useMemo(
    () => legacyDecoderDatasets,
    [legacyDecoderDatasets]
  );

  const selected = ALL_COMMANDS.find((entry) => entry.name === selectedCommand) ?? ALL_COMMANDS[0];
  ...
  const sampledVisibleFrames = useMemo(
    () => visibleFrames,
    [visibleFrames]
  );
  ...
  const selectedDecoderDataset = useMemo(
    () =>
      ...
      allDecoderDatasets[0] ??
      {
        ...
        dataset: {
          dataset_source: { vehicle: "Tesla", firmware: "none", mcu: "none", soc: "none" },
          frames: [],
        },
      },
      ...
      sampledVisibleFrames
        .map((frame) => {
          ...
          return {
            ...
            signalCount: decoded.reduce((total, entry) => total + entry.signals.length, 0),
          };
        })
        .filter((entry): entry is { frameKey: string; idHex: string; frameName: string; signalCount: number } => Boolean(entry)),
  ...
  const ensureSharedTransportReady = async (): Promise<void> => {
    ...
    if (matchesSelectedTransport(selectedTransportType, connection.controller.activeTransportType)) {
      return;
    }
  ...
  const refreshBleStatus = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error((payload?.error as string) || raw || `BLE status failed (${response.status})`);
      }
      ...
      pushDiagnosticsArchive("system", "BLE status fetched", `enabled=${enabled} connected=${connected}`);
  ...
  const applyBleDeviceName = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error((payload?.error as string) || raw || `BLE name update failed (${response.status})`);
      }
  ...
  const runCommand = async (name: CommandName, rawArgs = "") => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Failed to initialize selected transport.";
    ...
    if (!readiness.canExecute) {
      pushDiagnosticsArchive("command", `${name} blocked`, readiness.blockReason ?? "Command blocked", false);
    ...
    if (executionResult.shouldApplyBoardPayload && controllerResult.responseData !== undefined) {
  ...
  const runRawConsoleCommand = async (rawCommand: string): Promise<string> => {
    ...
    const routeLabel = selectedTransport === "http"
      ? "WiFi REST API"
      : selectedTransport === "ble"
        ? "BLE"
        : "COM CLI";
    const responseText = result.ok ? result.responseText ?? "ok" : result.error ?? "unknown failure";
    ...
    void (async () => {
      try {
        ...
        if (!result.ok) {
          ...
          pushDiagnosticsArchive("system", "Board info live toggle failed", reason, false);
          return;
        }
        ...
        const message = error instanceof Error ? error.message : "Board info live toggle failed.";
  ...
  const readStatus = async () => {
    try {
      ...
      const reason = error instanceof Error ? error.message : "Failed to initialize selected transport.";
    ...
    try {
      ...
      setStatusText(text);
      pushDiagnosticsArchive("system", "Status fetched", "Board status updated from transport.");
    } catch (error) {
  ...
  }, [activeTab, autoPoll, pollIndex, baseUrl, commandPath, statusPath, canFetchStatus, transportExecutionPolicy.blockReason]);
  ...
  const clearMonitorFeed = () => {
    ...
    pushDiagnosticsArchive("snapshot", "Feed cleared", "Cleared live frame feed and notifications.");
  };
  ...
  const exportVisibleFramesJson = () => {
    ...
    setLastResult(`Exported ${rowCount} visible frames as JSON (${EXPORT_SCHEMA_VERSION}, checksum=${checksum}, valid=${valid}).`);
    pushHistory({ command: "monitor:export:json", ok: valid, response: `rows=${rowCount} checksum=${checksum} schema=${EXPORT_SCHEMA_VERSION}` });
  };
  ...
  const exportVisibleFramesCsv = () => {
    ...
    const valid = verifyExportIntegrity(rows, checksum, rowCount);
    setStatusText(payload);
    setLastResult(`Exported ${rowCount} visible frames as CSV (${EXPORT_SCHEMA_VERSION}, checksum=${checksum}, valid=${valid}).`);
    pushHistory({ command: "monitor:export:csv", ok: valid, response: `rows=${rowCount} checksum=${checksum} schema=${EXPORT_SCHEMA_VERSION}` });
  };
  ...
  const exportRawSessionJson = () => {
    ...
    pushHistory({ command: "monitor:export:raw-json", ok: true, response: `rows=${rows.length}` });
  };
  ...
  const exportRawSessionJsonl = () => {
    ...
    const lines = boardState.frames.map((frame) => JSON.stringify({
      ts: frame.ts,
      idHex: `0x${frame.id.toString(16).toUpperCase()}`,
      bus: frame.bus,
      busName: frame.busName,
    ...
    pushHistory({ command: "monitor:export:raw-jsonl", ok: true, response: `rows=${lines.length}` });
  };
  ...
  const exportDecodedSessionJson = () => {
    ...
    setStatusText(payload);
    setLastResult(`Exported decoded session JSON (${rows.length} rows, dataset=${selectedDecoderDataset.id}).`);
    pushHistory({ command: "monitor:export:decoded-json", ok: true, response: `rows=${rows.length} dataset=${selectedDecoderDataset.id}` });
  };
  ...
  const exportDecodedSessionCsv = () => {
    ...
    for (const frame of visibleFrames) {
      const decoded = describeDecodedFrame(decoderIndex, frame.id);
      if (decoded.length === 0) {
        lines.push([frame.ts, `0x${frame.id.toString(16).toUpperCase()}`, selectedDecoderDataset.id, "", "", ""].map((part) => `"${String(part).replace(/"/g, '""')}"`).join(","));
        continue;
      }
      for (const entry of decoded) {
        if (entry.signals.length === 0) {
          lines.push([frame.ts, `0x${frame.id.toString(16).toUpperCase()}`, selectedDecoderDataset.id, entry.frameName, "", ""].map((part) => `"${String(part).replace(/"/g, '""')}"`).join(","));
          continue;
        }
        for (const signal of entry.signals) {
          const enumValues = signal.values.map((value) => `${value.value}:${value.label}`).join(" | ");
          lines.push([frame.ts, `0x${frame.id.toString(16).toUpperCase()}`, selectedDecoderDataset.id, entry.frameName, signal.name, enumValues].map((part) => `"${String(part).replace(/"/g, '""')}"`).join(","));
        }
      }
    }
    const meta = [
    ...
    const payload = `${meta}\n${header}\n${lines.join("\n")}`;
    setStatusText(payload);
    setLastResult(`Exported decoded session CSV (${lines.length} rows, dataset=${selectedDecoderDataset.id}).`);
    pushHistory({ command: "monitor:export:decoded-csv", ok: true, response: `rows=${lines.length} dataset=${selectedDecoderDataset.id}` });
  };
  ...
  const exportDecoderDatasetDbc = () => {
    ...
    lines.push(`CM_ \"Schema ${provenance.schemaVersion} exportedAt ${provenance.exportedAt} dataset ${provenance.dataset.id}\";`);
    lines.push("CM_ \"Dataset-derived DBC preview. Signal bit layout is not inferred from live traffic; names/enums come from selected dataset metadata.\";");
    for (const frame of selectedDecoderDataset.dataset.frames) {
      ...
      if (frame.signals.length === 0) {
        lines.push(` CM_ BO_ ${frame.id} \"No signal metadata available in selected dataset.\";`);
        continue;
      }
      frame.signals.forEach((signal, idx) => {
        const startBit = idx * 8;
        lines.push(` SG_ ${signal.signal_name} : ${startBit}|8@1+ (1,0) [0|255] \"\" MONITOR`);
        if (signal.possible_values && signal.possible_values.length > 0) {
          const enumDefs = signal.possible_values.map((value) => `${value.value_dec} \"${value.label.replace(/"/g, '\\"')}\"`).join(" ");
    ...
    setStatusText(payload);
    setLastResult(`Exported dataset-derived DBC preview (${selectedDecoderDataset.dataset.frames.length} frames, dataset=${selectedDecoderDataset.id}).`);
    pushHistory({ command: "monitor:export:dataset-dbc", ok: true, response: `frames=${selectedDecoderDataset.dataset.frames.length} dataset=${selectedDecoderDataset.id}` });
  };
  ...
  const exportSessionPackage = () => {
    ...
    setLastResult(`Exported session package (raw=${rawRows.length}, decoded=${decodedRows.length}, snapshots=${frameSnapshots.length}).`);
    pushHistory({ command: "monitor:export:session-package", ok: true, response: `raw=${rawRows.length} decoded=${decodedRows.length} snapshots=${frameSnapshots.length}` });
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {/* Dashboard tab */}
      {activeTab === "dashboard" ? (
        <DashboardScreen boardState={boardState} />
      ) : null}
      ...
      {activeTab === "console" ? (
        <ConsoleScreen
          ...
          decoderDatasets={allDecoderDatasets.map((entry) => ({ id: entry.id, label: entry.label }))}
          ...
          onDiagnosticsCategoryChange={(value) => setDiagnosticsCategory(value as DiagnosticsCategory)}
...
function ActionButton({
  ...
  return (
    ...
    >
      <Text style={[styles.buttonText, variant === "secondary" ? styles.buttonTextSecondary : undefined, disabled ? styles.buttonTextDisabled : undefined]}>
        {title}
      </Text>
    </Pressable>
  );
