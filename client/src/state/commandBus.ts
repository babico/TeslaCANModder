		...
		case "PURGE_SETTLED": {
			...
			return {
				...
				entries: state.entries.filter((e) => {
					...
					if (cutoff === null || cutoff === undefined) return false; // purge all settled
					return (
						e.finishedAt !== null &&
						e.finishedAt !== undefined &&
						now - e.finishedAt < cutoff
					);
				}),
			};
		}
...
export function selectPendingCommands(state: CommandBusState): CommandLifecycleEntry[] {
	return state.entries.filter((e) => e.status === "pending");
}

/** Returns entries that have settled (acked or failed). */
export function selectSettledCommands(state: CommandBusState): CommandLifecycleEntry[] {
	return state.entries.filter((e) => e.status !== "pending");
}

/** Returns the most recent entry, regardless of status. */
export function selectLastCommand(state: CommandBusState): CommandLifecycleEntry | undefined {
