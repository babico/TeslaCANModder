...
export function buildDiagnosticsEvents(input: BuildDiagnosticsEventsInput): DiagnosticsEvent[] {
	...
	return [...commandLifecycleEvents, ...commandHistoryEvents, ...boardEvents, ...snapshotEvents]
		.sort((a, b) => b.ts - a.ts)
		.slice(0, maxEvents);
}

export function filterDiagnosticsEvents(input: FilterDiagnosticsEventsInput): DiagnosticsEvent[] {
