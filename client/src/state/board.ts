...
function normalizeFrame(value: unknown): CanFrame | null {
  ...
  return {
    ...
    bus: Number.isFinite(bus) ? bus : 0,
    busName: typeof value.busName === "string" ? value.busName : `Bus${bus}`,
...
export function coerceBoardStateSnapshot(raw: unknown): BoardState | null {
  ...
  return {
    ...
    messages,
    frameCount:
      typeof raw.frameCount === "number" ? raw.frameCount : Array.isArray(frames) ? frames.length : 0,
