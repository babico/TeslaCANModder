...
export function BoardConnectionProvider({ children }: { children: ReactNode }) {
	...
	const runCommand = useCallback(
		async (name: CommandName, rawArgs = ""): Promise<MonitorCommandResult | null> => {
	...
	const disconnectTransport = useCallback(async () => {
		...
		try {
			...
			setLastResult("Transport disconnected");
			setStatusText("Transport disconnected");
		} catch (error) {
			const message =
				error instanceof Error ? error.message : "Failed to disconnect transport.";
			setLastResult(message);
			setStatusText(message);
		} finally {
			setConnectionBusy(false);
	...
	return (
		<BoardConnectionContext.Provider value={value}>{children}</BoardConnectionContext.Provider>
	);
}
