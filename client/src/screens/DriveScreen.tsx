...
function buildGlow(color: string, opacity: number, blur: number) {
  ...
  return { shadowColor: color, shadowOffset: { width: 0, height: 0 }, shadowOpacity: opacity, shadowRadius: blur };
}
...
export function resolveApState(tier: number, dasHandsOn: number, chassisOnline: boolean): ApClusterState {
...
const GEAR_DEFS: [number, string][] = [[1, "P"], [3, "N"], [4, "D"], [2, "R"]];
...
function GearLetter({ id, active }: { id: number; active: boolean }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(scale,   { toValue: active ? 1.22 : 1,  duration: motion.duration.fast, useNativeDriver }),
      Animated.timing(opacity, { toValue: active ? 1 : 0.18,  duration: motion.duration.fast, useNativeDriver }),
    ]).start();
  }, [active, scale, opacity]);

  const label = GEAR_DEFS.find(([g]) => g === id)?.[1] ?? "?";
  const color = !active      ? C.dim
    : id === 2 ? colors.gearReverse
    : id === 1 ? colors.gearPark
    : id === 3 ? colors.gearInactive
    : C.val;
...
function GearRow({ gear }: { gear: number }) {
  return (
    <View style={grS.row}>
      {GEAR_DEFS.map(([id]) => <GearLetter key={id} id={id} active={id === gear} />)}
    </View>
  );
}
...
function GaugeLine({ label, value, pct, color, muted }: {
  label: string; value: string; pct: number; color: string; muted?: boolean;
}) {
  ...
  return (
    <View style={glS.wrap}>
      ...
      <View style={glS.track}>
        <View style={[glS.fill, { width: w, backgroundColor: color, opacity: muted ? 0.2 : 1 }]} />
      </View>
    </View>
  );
}
...
const glS = StyleSheet.create({
  ...
  label:  { color: C.lbl, fontSize: font.size.xs, fontWeight: font.weight.bold, textTransform: "uppercase", letterSpacing: 0.5 },
...
function PowerBar({ power, maxKw = 200, maxDrive, maxRegen, onReset }: {
  power: number; maxKw?: number; maxDrive?: number; maxRegen?: number; onReset?: () => void;
}) {
  const abs   = Math.min(Math.abs(power), maxKw);
  const pct   = abs / maxKw;
  const isReg = power < 0;
  const dPct  = maxDrive != null ? Math.min(maxDrive / maxKw, 1) * 100 : null;
  const rPct  = maxRegen != null ? Math.min(maxRegen / maxKw, 1) * 100 : null;

  return (
    <View style={pbS.wrap}>
      <View style={pbS.track}>
        <View style={[pbS.fill, { width: `${Math.floor(pct * 100)}%` as `${number}%`, backgroundColor: isReg ? C.cyan : C.grn }]} />
        {dPct != null && dPct > 0 ? <View style={[pbS.mark, { left: `${Math.floor(dPct)}%` as `${number}%`, backgroundColor: C.grn }]} /> : null}
        {rPct != null && rPct > 0 ? <View style={[pbS.mark, { left: `${Math.floor(rPct)}%` as `${number}%`, backgroundColor: C.cyan }]} /> : null}
      </View>
      <View style={pbS.row}>
        <Text style={[pbS.label, { color: isReg ? C.cyan : C.grn }]}>
          {isReg ? `▼ ${Math.abs(power).toFixed(0)} kW  regen` : `▲ ${power.toFixed(0)} kW`}
        </Text>
        {onReset ? <Pressable onPress={onReset} hitSlop={8}><Text style={pbS.reset}>RST</Text></Pressable> : null}
      </View>
    </View>
  );
}
...
const pbS = StyleSheet.create({
  wrap:  { gap: 3 },
  track: { height: 6, borderRadius: radius.full, backgroundColor: C.barBg, overflow: "hidden", position: "relative" },
  fill:  { height: "100%", borderRadius: radius.full },
...
function SpeedDial({ state, gaugeMode, onCycle, size = 220 }: {
  state: BoardState; gaugeMode: GaugeMode; onCycle: () => void; size?: number;
}) {
  const { cycleUnit, convert, label: unitLabel } = useSpeedUnit();
  const speed  = Number.isFinite(state.vehicleSpeed) ? convert(state.vehicleSpeed).toFixed(0) : "—";
  const speedFs = size < 200 ? 52 : 68;
  const pedal  = Math.max(0, Math.min(100, state.accelPedal));
  const pwr    = state.bmsPower;

  const ringColor = Number.isFinite(pwr) && pwr > 5  ? C.grn
    : Number.isFinite(pwr) && pwr < -5 ? C.cyan
    : C.bord;
  ...
  return (
    <View style={[sdS.ring, { width: size, height: size, borderRadius: size / 2, borderColor: ringColor, ...buildGlow(ringColor, glowOpacity, 28) }]}>
      <Pressable onPress={cycleUnit} hitSlop={12}>
        <Text style={sdS.unit}>{unitLabel}</Text>
      </Pressable>
      <Pressable onPress={onCycle} hitSlop={8}>
        <Text style={[sdS.speed, { fontSize: speedFs, lineHeight: speedFs + 4 }]}>{speed}</Text>
      </Pressable>
      ...
      {gaugeMode === "power" ? (
        ...
        <View style={sdS.sub}>
          <View style={sdS.miniTrack}>
            ...
            <View style={[sdS.miniDot, { left: `${50 + steerNorm * 44}%` as `${number}%` }]} />
          </View>
          <Text style={sdS.subLbl}>{(state.steeringAngle ?? 0).toFixed(0)}°  ·  {pedal.toFixed(0)}%</Text>
        </View>
      ) : (
        <View style={sdS.sub}>
          <View style={sdS.miniTrack}>
...
const sdS = StyleSheet.create({
  ring:      { alignItems: "center", justifyContent: "center", gap: 4, borderWidth: 3, backgroundColor: C.card, elevation: 16 },
  unit:      { color: C.dim, fontSize: font.size.xs, fontWeight: font.weight.bold, letterSpacing: 2, textTransform: "uppercase" },
  speed:     { color: C.val, fontWeight: font.weight.black, letterSpacing: -3 },
  sub:       { alignItems: "center", gap: 3, width: "68%" },
  miniTrack: { width: "100%", height: 5, borderRadius: radius.full, backgroundColor: C.barBg, overflow: "hidden", position: "relative" },
  miniFill:  { height: "100%", backgroundColor: C.cyan, borderRadius: radius.full },
  miniCenter:{ position: "absolute", left: "50%", width: 1, height: "100%", backgroundColor: C.lbl },
  miniDot:   { position: "absolute", width: 8, height: "100%", marginLeft: -4, backgroundColor: C.cyan, borderRadius: radius.full },
  subLbl:    { color: C.dim, fontSize: font.size.xs, fontWeight: font.weight.bold, letterSpacing: 0.5, textTransform: "uppercase" },
...
function LeftPanel({ state }: { state: BoardState }) {
  ...
  const range    = state.bmsIdealRemaining > 0  ? state.bmsIdealRemaining
    : state.bmsExpectedRange > 0 ? state.bmsExpectedRange
    : null;
  const pwr = state.bmsPower;
  const pwrColor = Number.isFinite(pwr) && pwr < 0 ? C.cyan : Number.isFinite(pwr) && pwr > 5 ? C.grn : C.sub;
  ...
  return (
    <View style={lpS.panel}>
      ...
      <View style={lpS.socTrack}>
        <View style={[lpS.socFill, { width: `${soc}%` as `${number}%`, backgroundColor: socColor }]} />
      </View>
      {range != null ? (
        <Text style={lpS.range}>{range.toFixed(0)} km est</Text>
      ) : null}
      {Number.isFinite(pwr) ? (
      ...
      {state.hasPowertrain ? (
        <>
          ...
          <GaugeLine
            label="FRONT"
            value={Math.abs(state.frontMotorRpm) > 5 ? `${Math.abs(state.frontMotorRpm).toFixed(0)} rpm` : "RWD"}
...
const lpS = StyleSheet.create({
  panel:     { flex: 1, paddingHorizontal: spacing.sm, paddingVertical: spacing.sm, gap: spacing.xs2 + 2, justifyContent: "center" },
  ...
  socMode:   { color: C.lbl, fontSize: font.size.xs, fontWeight: font.weight.bold, letterSpacing: 1, textTransform: "uppercase" },
  socTrack:  { height: 8, borderRadius: radius.full, backgroundColor: C.barBg, overflow: "hidden" },
...
function TurnChip({ side, bsmLevel, turnOn }: { side: "L" | "R"; bsmLevel: number; turnOn: boolean }) {
...
const tcS = StyleSheet.create({
  chip:  { flexDirection: "row", alignItems: "center", gap: spacing.xs, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs2 + 2, borderWidth: 1, borderRadius: radius.md, backgroundColor: C.card },
  arrow: { fontSize: font.size.lg, fontWeight: font.weight.black },
  side:  { color: C.lbl, fontSize: font.size.xs, fontWeight: font.weight.bold, letterSpacing: 1, textTransform: "uppercase" },
  label: { fontSize: font.size.xs, fontWeight: font.weight.extrabold, letterSpacing: 0.5, textTransform: "uppercase", marginLeft: "auto" },
  ...
  return (
    <View style={dcS.wrap}>
      ...
      <View style={dcS.body}>
        ...
        <View style={dcS.hlL} /><View style={dcS.hlR} />
        <View style={dcS.tlL} /><View style={dcS.tlR} />
      </View>
    </View>
  );
}
...
const dcS = StyleSheet.create({
  ...
  shadow:   { position: "absolute", bottom: 4, width: 36, height: 4, borderRadius: radius.full, backgroundColor: C.bord, opacity: 0.5 },
  body:     { width: 40, height: 74, borderRadius: radius.lg, borderWidth: 1, borderColor: C.sub, backgroundColor: C.card, position: "relative", overflow: "hidden" },
  roof:     { position: "absolute", left: 9, top: 20, width: 22, height: 34, borderWidth: 1, borderColor: C.bord, borderRadius: radius.sm },
  ...
  hlL: { position: "absolute", left: 7,  top: 1,    width: 6, height: 2, borderRadius: radius.full, backgroundColor: C.amb },
  hlR: { position: "absolute", right: 7, top: 1,    width: 6, height: 2, borderRadius: radius.full, backgroundColor: C.amb },
  tlL: { position: "absolute", left: 7,  bottom: 1, width: 6, height: 2, borderRadius: radius.full, backgroundColor: C.red },
  tlR: { position: "absolute", right: 7, bottom: 1, width: 6, height: 2, borderRadius: radius.full, backgroundColor: C.red },
});
    ...
    <View style={rpS.panel}>
      ...
      <View style={rpS.doorsRow}>
        ...
        <View style={rpS.doorsText}>
          ...
          {hasOpen ? <Text style={rpS.doorsList} numberOfLines={2}>{openDoors.join(" · ")}</Text> : null}
        </View>
      </View>
      <View style={rpS.divider} />
...
const rpS = StyleSheet.create({
  panel:      { flex: 1, paddingHorizontal: spacing.sm, paddingVertical: spacing.sm, gap: spacing.xs, justifyContent: "center" },
  divider:    { height: 1, backgroundColor: C.bord },
  doorsRow:   { flexDirection: "row", alignItems: "center", gap: spacing.sm },
  doorsText:  { flex: 1, gap: 2 },
  secTitle:   { color: C.lbl, fontSize: font.size.xs, fontWeight: font.weight.bold, letterSpacing: 1, textTransform: "uppercase" },
  doorsState: { fontSize: font.size.md, fontWeight: font.weight.extrabold, textTransform: "uppercase", letterSpacing: 0.3 },
  doorsList:  { color: C.sub, fontSize: font.size.xs },
  cruise:     { gap: 2 },
  cruiseState:{ fontSize: font.size.md, fontWeight: font.weight.extrabold, textTransform: "uppercase", letterSpacing: 0.3 },
...
function TopBar({
  state, showHud, onToggleHud,
}: { state: BoardState; showHud?: boolean; onToggleHud?: () => void }) {
  ...
  return (
    <View style={tbS.bar}>
      <View style={tbS.left}>
        ...
        <Text style={tbS.sep}>·</Text>
        <Text style={tbS.mode}>{formatDriveMode(state.currentDriveMode ?? state.driveMode)}</Text>
      </View>
      <View style={tbS.right}>
        ...
        <Badge variant={apBadgeVariant(apState)}>{apStateLabel(apState, state.gtwAutopilotTier)}</Badge>
        {onToggleHud ? (
          <Pressable onPress={onToggleHud} style={[tbS.hudBtn, showHud ? tbS.hudBtnOn : tbS.hudBtnOff]} hitSlop={8}>
            <Text style={[tbS.hudTxt, showHud ? tbS.hudTxtOn : tbS.hudTxtOff]}>{showHud ? "▲" : "▼"}</Text>
          </Pressable>
        ) : null}
        {source !== "force" ? (
          <Pressable onPress={() => setOverride(isDark ? "light" : "dark")} hitSlop={8} style={tbS.themeBtn}>
...
const tbS = StyleSheet.create({
  bar:      { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: spacing.md, paddingVertical: spacing.sm, backgroundColor: C.card, borderBottomWidth: 1, borderBottomColor: C.bord, minHeight: 40 },
  ...
  mode:     { color: C.sub, fontSize: font.size.xs, fontWeight: font.weight.semibold, textTransform: "uppercase", flexShrink: 1 },
  hudBtn:   { paddingHorizontal: spacing.xs, paddingVertical: 2, borderRadius: radius.sm, borderWidth: 1 },
...
const hiS = StyleSheet.create({
  row:  { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, paddingHorizontal: spacing.md, paddingVertical: spacing.sm2, backgroundColor: C.card, borderBottomWidth: 1, borderBottomColor: C.bord },
...
function BottomStrip({ state, maxDrive, maxRegen, onReset }: {
  state: BoardState; maxDrive?: number; maxRegen?: number; onReset: () => void;
}) {
  ...
  return (
    <View style={bsS.strip}>
      <PowerBar power={state.bmsPower} maxDrive={maxDrive} maxRegen={maxRegen} onReset={onReset} />
      <View style={bsS.chips}>
        <StatChip label="Mode"  value={driveMode}                    variant="dark" accent={driveMode !== "OFF"} />
        <StatChip label="FSD"   value={state.fsd ? "ON" : "OFF"}     variant="dark" accent={state.fsd} />
        {state.hasBms ? (
          <>
            <StatChip label="Regen" value={state.bmsMaxRegenPower.toFixed(0)} unit="kW" variant="dark" />
            <StatChip label="Temp"  value={`${state.bmsTempMin.toFixed(0)}–${state.bmsTempMax.toFixed(0)}`} unit="°C" variant="dark" />
          </>
        ) : null}
        {tires.map(([lbl, p]) => {
          const missing = p <= 0;
...
const bsS = StyleSheet.create({
  strip: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, gap: spacing.sm, backgroundColor: C.strip, borderTopWidth: 1, borderTopColor: C.bord },
...
const ds = StyleSheet.create({
  ...
  lsCenter: { flex: 1, alignItems: "center", justifyContent: "center", paddingVertical: spacing.sm },
  lsRight:  { width: 178, borderLeftWidth: 1, borderLeftColor: C.bord },

  // portrait
  ptCenter: { alignItems: "center", justifyContent: "center", paddingVertical: spacing.md, borderBottomWidth: 1, borderBottomColor: C.bord },
