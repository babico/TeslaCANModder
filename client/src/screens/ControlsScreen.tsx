...
const FEATURE_BUS_HINTS: Partial<Record<FeatureId, { busLabel: string; busColor: string; busField?: BusField }>> = {
...
function buildFeatureCommandItems(featureId: FeatureId): CommandItem[] {
  ...
  for (const setting of spec.settings) {
    setting.commandNames.forEach((commandName, index) => {
      ...
      const optionLabel = setting.control === "enum" ? setting.options[index]?.label : undefined;
...
const GROUPS: CommandGroup[] = FEATURE_IDS
  .map((featureId) => {
    const spec = FEATURE_SETTINGS_BY_ID[featureId];
...
export function ControlsScreen({ boardState, onRunCommand }: ControlsScreenProps) {
  ...
  const profileSetting = getFeatureSettingsSpecById("speedProfile")?.settings.find((setting) => setting.id === "profile");
  ...
  const runWithGuards = (cmd: CommandItem, gateReason?: string | null) => {
    ...
    if (argCount > 0 && !hasToggleDefault) {
      setPendingConfirm(null);
      setTooltip(`${cmd.label} requires input. Use Monitor > Command Runner for this command.`);
      return;
    }
  ...
  const importantFeatures = [
    ...
    },
    {
      ...
      secondary: { label: "Disable", action: () => runDirectCommand("precondition", "false") },
    },
    {
      title: "Summon",
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.topGrid}>
        <View style={styles.panelCard}>
          <View style={styles.panelHeader}>
            ...
            <View style={styles.badgeAccent}>
              <Text style={styles.badgeAccentText}>{String(boardState.variant || "unknown").toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.healthBanner}>
            <View style={[styles.healthDot, { backgroundColor: boardState.chassisOnline || boardState.vehicleOnline || boardState.bodyOnline ? colors.powerPositive : colors.statusDisconnected }]} />
            <View style={styles.healthCopy}>
              <Text style={styles.healthTitle}>{boardState.otaInProgress ? "Standby" : "CAN Active"}</Text>
              <Text style={styles.healthSubtitle}>{boardState.otaInProgress ? "TX paused during OTA/update state" : "Vehicle buses responding"}</Text>
            </View>
          </View>
          <View style={styles.statGrid}>
            <View style={styles.statCell}><Text style={styles.statLabel}>Board</Text><Text style={styles.statValue}>{boardState.hardware || "-"}</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Driver</Text><Text style={styles.statValue}>{boardState.driver || "-"}</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Uptime</Text><Text style={styles.statValue}>{Math.max(0, Math.floor((boardState.uptime || 0) / 1000))}s</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Detected HW</Text><Text style={styles.statValue}>{boardState.detectedHW > 0 ? `HW${boardState.detectedHW}` : "Auto"}</Text></View>
          </View>
        </View>
        ...
        <View style={styles.panelCard}>
          <View style={styles.panelHeader}>
            ...
            <View style={styles.badgeMuted}><Text style={styles.badgeMutedText}>Saved</Text></View>
          </View>
          <View style={styles.settingsGrid}>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>Variant</Text><Text style={styles.settingsValue}>{String(boardState.variant || "unknown").toUpperCase()}</Text></View>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>FSD</Text><Text style={[styles.settingsValue, boardState.fsd ? styles.valueOn : styles.valueOff]}>{boardState.fsd ? "ON" : "OFF"}</Text></View>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>Nag</Text><Text style={[styles.settingsValue, boardState.nag ? styles.valueOn : styles.valueOff]}>{boardState.nag ? "ON" : "OFF"}</Text></View>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>{profileLevelLabel}</Text><Text style={styles.settingsValue}>{profileValue} {boardState.profilePinned ? "(pinned)" : "(auto)"}</Text></View>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>Offset</Text><Text style={styles.settingsValue}>{offsetValue}% {boardState.offsetPinned ? "(pinned)" : "(auto)"}</Text></View>
            <View style={styles.settingsItem}><Text style={styles.settingsKey}>ISA</Text><Text style={styles.settingsValue}>{boardState.isaChime ? "Suppress" : "Original"}</Text></View>
          </View>
        </View>
      </View>
        ...
        {importantFeatures.map((feature) => (
          <View key={feature.title} style={styles.featureTile}>
            <View style={styles.featureTileHeader}>
              ...
              <Text style={[styles.featureStatus, feature.active ? styles.statusOn : styles.statusOff]}>{feature.activeLabel}</Text>
            </View>
            <View style={styles.featureTileActions}>
              <Pressable style={({ pressed }) => [styles.featureButton, feature.active ? styles.featureButtonActive : undefined, pressed ? styles.featureButtonPressed : undefined]} onPress={feature.primary.action}>
                <Text style={[styles.featureButtonText, feature.active ? styles.featureButtonTextActive : undefined]}>{feature.primary.label}</Text>
              </Pressable>
              <Pressable style={({ pressed }) => [styles.featureButton, !feature.active && !feature.tertiary ? styles.featureButtonActive : undefined, pressed ? styles.featureButtonPressed : undefined]} onPress={feature.secondary.action}>
                <Text style={[styles.featureButtonText, !feature.active && !feature.tertiary ? styles.featureButtonTextActive : undefined]}>{feature.secondary.label}</Text>
              </Pressable>
              {feature.tertiary ? (
                <Pressable style={({ pressed }) => [styles.featureButton, styles.featureButtonDanger, pressed ? styles.featureButtonPressed : undefined]} onPress={feature.tertiary.action}>
                  <Text style={styles.featureButtonTextActive}>{feature.tertiary.label}</Text>
                </Pressable>
              ) : null}
            </View>
          </View>
      ...
      {(boardState.otaInProgress || boardState.txPaused || boardState.detectedHW > 0) ? (
        <View style={styles.panelCard}>
          ...
          <View style={styles.settingsGrid}>
            {boardState.otaInProgress ? <View style={styles.settingsItem}><Text style={styles.settingsKey}>OTA</Text><Text style={[styles.settingsValue, styles.valueDanger]}>In Progress</Text></View> : null}
            {boardState.txPaused ? <View style={styles.settingsItem}><Text style={styles.settingsKey}>TX</Text><Text style={[styles.settingsValue, styles.valueDanger]}>Paused</Text></View> : null}
            {boardState.detectedHW > 0 ? <View style={styles.settingsItem}><Text style={styles.settingsKey}>Detected HW</Text><Text style={styles.settingsValue}>HW{boardState.detectedHW}</Text></View> : null}
          </View>
        </View>
      ) : null}
      ...
      {boardState.hasBms ? (
        <View style={styles.panelCard}>
          <View style={styles.panelHeader}>
            ...
            <View style={styles.badgeAccent}><Text style={styles.badgeAccentText}>Live</Text></View>
          </View>
          <View style={styles.statGrid}>
            <View style={styles.statCell}><Text style={styles.statLabel}>Voltage</Text><Text style={styles.statValue}>{bmsVoltage.toFixed(1)}V</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Current</Text><Text style={styles.statValue}>{bmsCurrent.toFixed(1)}A</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Power</Text><Text style={styles.statValue}>{bmsPower.toFixed(1)}kW</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>SoC</Text><Text style={styles.statValue}>{bmsSoc.toFixed(1)}%</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Temp Min</Text><Text style={styles.statValue}>{bmsTempMin}°C</Text></View>
            <View style={styles.statCell}><Text style={styles.statLabel}>Temp Max</Text><Text style={styles.statValue}>{bmsTempMax}°C</Text></View>
          </View>
        </View>
      ) : null}
