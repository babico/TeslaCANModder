...
type BoardType = "arduino" | "esp32";
type StatusType = "idle" | "building" | "connecting" | "flashing" | "done" | "error";

type ConnectionOption = {
  key: "serial" | "bt" | "wifi" | "ble";
  label: string;
  desc: string;
  locked?: boolean;
};

const CONNECTION_ARDUINO: ConnectionOption[] = [
...
function resolveEnvironment(board: BoardType, connection: Record<string, number>): string {
  if (board === "arduino") {
    return connection.bt ? "uno_bt" : "uno";
  }
...
async function buildAndDownload(
  ...
  const endpoint = apiBaseUrl ? `${apiBaseUrl}/api/build` : "/api/build";
  ...
  if (!response.ok) {
    const errorPayload = await response.json().catch(() => ({ error: "Build failed" }));
...
export function FlasherScreen() {
  ...
  const apiBaseUrl = (sharedConnection.baseUrl ?? "").trim().replace(/\/+$/, "");
  const buildApiBaseUrl = sharedConnection.selectedTransportType === "http" ? apiBaseUrl : "";
  const [board, setBoard] = useState<BoardType>("arduino");
  const [connection, setConnection] = useState<Record<string, number>>({ serial: 1, bt: 0, wifi: 0, ble: 0 });
  ...
  const connectionOptions = board === "arduino" ? CONNECTION_ARDUINO : CONNECTION_ESP32;
  const environment = useMemo(() => resolveEnvironment(board, connection), [board, connection]);
  const extension: ".hex" | ".bin" = board === "arduino" ? ".hex" : ".bin";
  ...
  const switchBoard = (nextBoard: BoardType) => {
    setBoard(nextBoard);
    setConnection({ serial: 1, bt: 0, wifi: 0, ble: 0 });
    setBuses({ chassis: 1, vehicle: 0, body: 0 });
    setStatus("idle");
    setMessage("");
  };
  ...
  const handleBuild = async () => {
    ...
    try {
      const filename = await buildAndDownload(buildApiBaseUrl, environment, extension, buses);
      setStatus("done");
  ...
  const handleFlash = async () => {
    ...
    try {
      ...
      if (!isSerialFamilyTransport(activeTransport)) {
        throw new Error("Shared USB serial must already be connected. Open shared connection first, then start flash.");
      }
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <View style={styles.hero}>
        ...
        <Text style={styles.subtitle}>Select board, connectivity, and CAN bus profile; then build/download or flash over USB.</Text>
      </View>

      <View style={styles.panel}>
        <Text style={styles.panelTitle}>Board</Text>
        <View style={styles.segmentRow}>
          <Pressable onPress={() => switchBoard("arduino")} style={[styles.segment, board === "arduino" ? styles.segmentActive : undefined]}>
            <Text style={[styles.segmentTitle, board === "arduino" ? styles.segmentTitleActive : undefined]}>Arduino Uno R3</Text>
            <Text style={styles.segmentDetail}>MCP2515 x1-3 · HC-05 BT · EEPROM</Text>
          </Pressable>
          <Pressable onPress={() => switchBoard("esp32")} style={[styles.segment, board === "esp32" ? styles.segmentActive : undefined]}>
            <Text style={[styles.segmentTitle, board === "esp32" ? styles.segmentTitleActive : undefined]}>ESP32-S DevKit</Text>
          ...
          {connectionOptions.map((option) => {
            ...
            return (
              <Pressable
                ...
                style={[styles.chip, active ? styles.chipActive : undefined, option.locked ? styles.chipLocked : undefined]}
              >
                <Text style={[styles.chipLabel, active ? styles.chipLabelActive : undefined]}>{option.label}</Text>
                <Text style={styles.chipDetail}>{option.desc}</Text>
                <Text style={[styles.chipStatus, active ? styles.chipStatusActive : undefined]}>{active ? "ON" : "OFF"}</Text>
              </Pressable>
            );
          })}
        </View>
        <Text style={styles.resolvedLine}>Resolved environment: {environment}</Text>
      </View>
        ...
        <View style={styles.chipGrid}>
          {BUS_OPTIONS.map((bus) => {
            ...
            return (
              ...
              >
                <Text style={[styles.chipLabel, active ? styles.chipLabelActive : undefined]}>{bus.label}</Text>
      ...
      <View style={styles.panel}>
        ...
        <Text style={styles.summaryLine}>{["Chassis", buses.vehicle ? "Vehicle" : null, buses.body ? "Body" : null].filter(Boolean).join(" + ")}</Text>
        <View style={styles.actionsRow}>
          <Pressable onPress={() => void handleBuild()} disabled={status === "building"} style={[styles.action, styles.primaryAction, status === "building" ? styles.actionDisabled : undefined]}>
            <Text style={styles.actionText}>{status === "building" ? "Building..." : "Build and Download"}</Text>
          </Pressable>
          <Pressable
            onPress={() => void handleFlash()}
            disabled={status === "building" || status === "connecting" || status === "flashing"}
            style={[styles.action, styles.secondaryAction, status === "building" || status === "connecting" || status === "flashing" ? styles.actionDisabled : undefined]}
          >
        ...
        {message ? <Text style={[styles.message, status === "error" ? styles.messageError : status === "done" ? styles.messageDone : undefined]}>{message}</Text> : null}
      </View>
        ...
        <View style={styles.panel}>
          ...
          >
            {flashLog.map((line, idx) => (
              <Text key={idx} style={styles.flashConsoleText}>{line}</Text>
            ))}
          </ScrollView>
        </View>
      ) : null}
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.codeBlock}>
          <Text style={styles.codeText}>cd hardware</Text>
          <Text style={styles.codeText}>PLATFORMIO_BUILD_FLAGS="{buildFlags}" pio run -e {environment}</Text>
