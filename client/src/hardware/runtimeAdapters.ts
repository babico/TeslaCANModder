...
function getWebSerialApi(): NonNullable<WebSerialNavigator["serial"]> {
  if (typeof navigator === "undefined") {
    throw new Error("Serial transport is not available in this native runtime. Use REST API here, or open the web client to use Web Serial.");
  }

  const serialApi = (navigator as WebSerialNavigator).serial;
  if (!serialApi) {
    throw new Error("Web Serial is not available in this runtime. Use Chrome or Edge, or switch back to REST API.");
  }
...
function getWebBluetoothApi(): NonNullable<WebBluetoothNavigator["bluetooth"]> {
  if (typeof navigator === "undefined") {
    throw new Error("BLE transport is not available in this native runtime. Use REST API here, or open the web client to use Web Bluetooth.");
  }

  const bluetoothApi = (navigator as WebBluetoothNavigator).bluetooth;
  if (!bluetoothApi) {
    throw new Error("Web Bluetooth is not available in this runtime. BLE transport currently works only in browser builds that expose navigator.bluetooth.");
  }
    ...
    try {
      while (Date.now() - startedAt < 5000) {
        ...
        while (true) {
          ...
          const nextOffset = this.readBuffer[lineBreakIndex] === "\r" && this.readBuffer[lineBreakIndex + 1] === "\n"
            ? lineBreakIndex + 2
            : lineBreakIndex + 1;
