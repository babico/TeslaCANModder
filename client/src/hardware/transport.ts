...
export class UnsupportedTransport implements CommandTransport {
  ...
  send(_command: string): Promise<unknown> {
    return Promise.reject(
      new Error("No transport available: cannot send command")
    );
  }

  status(): Promise<unknown> {
    return Promise.reject(
      new Error("No transport available: cannot read status")
    );
  }
}
...
export class BleCommandTransport implements CommandTransport {
  ...
  private withTimeout<T>(promise: Promise<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("BLE operation timed out")), this.timeoutMs);
      promise.then(
...
export interface BluetoothSerialTransportConfig extends SerialTransportConfig {}
...
export class SerialCommandTransport implements CommandTransport {
  ...
  private withTimeout<T>(promise: Promise<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Serial operation timed out")), this.timeoutMs);
      promise.then(
        (value) => { clearTimeout(timer); resolve(value); },
        (err) => { clearTimeout(timer); reject(err); }
      );
