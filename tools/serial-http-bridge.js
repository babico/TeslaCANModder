...
function printHelpAndExit(code) {
  console.log("Usage: node tools/serial-http-bridge.js --port COM4 [--baud 115200] [--listen 8080] [--host 127.0.0.1] [--timeout 3000]");
  process.exit(code);
}
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && url === "/api/status") {
        session.send("status");
        const entry = await session.waitFor((item) => item.msg?.t === "status" || item.msg?.t === "boot", options.timeoutMs);
        if (!entry) {
