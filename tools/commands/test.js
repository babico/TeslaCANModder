...
export async function runTest(session, opts, out) {
	...
	if (baseline)
		out.info(`Baseline — fsd=${origFsd} sp=${origProfile} variant=${baseline.msg.variant}`);
	...
	if (testFsd) {
		...
		if (onAck) out.pass("fsd:on acknowledged");
		else out.fail("fsd:on", "no ack");

		await delay(600);
		session.send("status");
		const afterOn = await session.waitForType("status", timeoutMs);
		if (afterOn) {
			if (Number(afterOn.msg.fsd) === 1)
				out.pass("FSD enabled in status", `fsd=${afterOn.msg.fsd}`);
			else out.fail("FSD on verification", `fsd=${afterOn.msg.fsd} (expected 1)`);
		...
		if (offAck) out.pass("fsd:off acknowledged");
		else out.fail("fsd:off", "no ack");

		await delay(600);
		session.send("status");
		const afterOff = await session.waitForType("status", timeoutMs);
		if (afterOff) {
			if (Number(afterOff.msg.fsd) === 0)
				out.pass("FSD disabled in status", `fsd=${afterOff.msg.fsd}`);
			else out.fail("FSD off verification", `fsd=${afterOff.msg.fsd} (expected 0)`);
	...
	if (testProfile >= 0) {
		...
		if (ack) out.pass(`${cmd} acknowledged`);
		else out.fail(cmd, "no ack");

		await delay(700);
		session.send("status");
		const afterSet = await session.waitForType("status", timeoutMs);
		if (afterSet) {
			if (Number(afterSet.msg.sp) === testProfile)
				out.pass(`Profile ${testProfile} confirmed`, `sp=${afterSet.msg.sp}`);
			else
				out.fail(
					`Profile ${testProfile} verification`,
					`sp=${afterSet.msg.sp} (expected ${testProfile})`,
				);
	...
	if (testFsdListen) {
		...
		const afterOn = await session.waitForType("status", timeoutMs);
		if (afterOn) {
			if (Number(afterOn.msg.fsd) === 1) out.pass("FSD enabled", `fsd=${afterOn.msg.fsd}`);
			else out.fail("FSD activation", `fsd=${afterOn.msg.fsd}`);
		}
