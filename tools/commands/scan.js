...
export async function runScan(session, opts, out, C) {
  ...
  while (Date.now() < deadline) {
    ...
    if (existing) {
      ...
      idMap.set(id, { count: 1, firstSeen: now, lastSeen: now, dirs: new Set([entry.msg.dir]), dlcs: new Set([entry.msg.dlc]) });
    }
  }
  ...
  for (const [id, info] of sorted) {
    ...
    console.log(`  ${C.cyan}${String(id).padEnd(8)}${C.reset} ${hex.padEnd(8)} ${String(info.count).padEnd(8)} ${dirs.padEnd(8)} ${dlcs.padEnd(8)} ${info.firstSeen}`);
  }

  console.log(`\n  ${C.bold}${idMap.size} unique CAN IDs${C.reset} detected in ${scanDurMs}ms`);
  out.pass(`Scan complete — ${idMap.size} unique IDs, ${[...idMap.values()].reduce((s, v) => s + v.count, 0)} total frames`);
}
