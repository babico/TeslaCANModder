...
export async function runWatch(session, opts, out, C) {
  ...
  if (watchRaw) {
    ...
    ack ? out.info('raw CAN mode enabled') : out.warn('can:raw:on', 'not acknowledged');
  }
  ...
  while (Date.now() < deadline) {
    ...
    const { msg, raw } = entry;
    if (!msg) { console.log(`${C.dim}[${ts()}] text: ${raw}${C.reset}`); continue; }
        ...
        if (msg.dir === 'tx') {
          ...
          if (changes.length > 0) {
            ...
            for (const { byteIdx, prevByte, currByte, changedBits } of changes) {
              console.log(`    ${C.bold}Byte ${byteIdx}${C.reset}: 0x${prevByte.toString(16).toUpperCase().padStart(2, '0')} → 0x${currByte.toString(16).toUpperCase().padStart(2, '0')}`);
              for (const { bit, from, to } of changedBits) console.log(`      ${C.yellow}bit ${bit}${C.reset}: ${from} → ${to}`);
            }
          }
    ...
    if (msg.t === 'status') {
      const newFsd = Number(msg.fsd), newProfile = Number(msg.sp), newReady = msg.ready || null;
      if (!watchState._seen) {
        watchState = { _seen: true, fsd: newFsd, profile: newProfile, ready: newReady, rawCan: Number(msg.rawCan), streamOn: msg.stream?.on };
        if (watchFsd || watchProfile) out.observation('status', `initial — ready=${newReady} fsd=${newFsd} sp=${newProfile}`);
        continue;
      }
      if (watchFsd && watchState.fsd !== newFsd) { out.observation('fsd', `${watchState.fsd} → ${newFsd}`); watchState.fsd = newFsd; }
      if (watchProfile && watchState.profile !== newProfile) { out.observation('profile', `${watchState.profile} → ${newProfile}`); watchState.profile = newProfile; }
      continue;
    }

    if (msg.t === 'ack') { out.observation('ack', msg.cmd || '?'); continue; }
    if (msg.t === 'error') { out.observation('error', msg.msg || '?'); continue; }
    if (msg.t === 'log') out.observation('log', msg.msg || raw);
  }

  session.send('stream:off');
  if (watchRaw) { session.send('can:raw:off'); await delay(200); }
  console.log('\nWatch ended.');
}
