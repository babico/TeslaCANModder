...
export async function runSmoke(session, opts, out) {
  ...
  if (boot) {
    out.pass('Boot message received', `hw=${boot.msg.hw || '?'} variant=${boot.msg.variant || '?'}`);
    if (boot.msg.cap) out.info(`capabilities: ${boot.msg.cap}`);
  } else {
    session.send('status');
    const status = await session.waitForType('status', timeoutMs);
    status
      ? out.pass('Board already running — status responded', `variant=${status.msg.variant || '?'}`)
      : out.fail('Boot', 'No boot or status message received within timeout');
  }

  out.section('Ping / pong');
  session.send('ping');
  const pong = await session.waitForType('pong', timeoutMs);
  pong ? out.pass('Pong received') : out.fail('Pong', 'No pong within timeout');
  ...
  if (status) {
    ...
    missing.length === 0
      ? out.pass('Status schema valid')
      : out.fail('Status schema', `missing fields: ${missing.join(', ')}`);
  } else {
  ...
  streamAck ? out.pass('stream:on acknowledged') : out.warn('stream:on', 'No ack — continuing');
  ...
  else if (frameCount > 0) out.warn(`Only ${frameCount} filtered frames`, 'few frames — board connected to vehicle?');
  ...
  if (rawAck) {
    ...
    rawFrames.length > 0
      ? out.pass(`${rawFrames.length} raw CAN frames seen`)
      : out.warn('No raw frames', 'check physical CAN bus connection');
  ...
  post?.msg?.variant === 'hw4'
    ? out.pass('Variant switch to hw4 confirmed')
    : out.fail('Variant switch', `status shows variant=${post?.msg?.variant || '?'}`);
