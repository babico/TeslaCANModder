...
export function assessDriveContextSnapshot(snapshot) {
  ...
  if (!Number.isFinite(snapshot.cruiseSetSpeedKph) ||
...
export function evaluateDriveClosureReadiness(coverage, consistencyOk) {
  ...
  return {
    ...
    notes: {
      d05: d05Ready ? 'Ready to close after evidence review' : 'Missing one or more D-05 coverage signals',
      d11: d11Ready ? 'Ready to close after evidence review' : 'Missing one or more D-11 opening-state signals',
      d13: d13Ready ? 'Ready to close after evidence review' : 'Missing cruise/max/limit evidence for D-13',
      consistency: consistencyOk ? 'No contradictions detected' : 'Consistency check failed; review errors',
    },
  };
}
...
export function buildDriveClosureChecklist(coverage, readiness) {
  ...
  if (!coverage.d05.turnLeftSeen) missing.d05.push({ signal: 'turnLeftSeen', scenarioId: '14.1' });
  if (!coverage.d05.turnRightSeen) missing.d05.push({ signal: 'turnRightSeen', scenarioId: '14.2' });
  if (!coverage.d05.bsmLeftNonZeroSeen) missing.d05.push({ signal: 'bsmLeftNonZeroSeen', scenarioId: '14.4' });
  if (!coverage.d05.bsmRightNonZeroSeen) missing.d05.push({ signal: 'bsmRightNonZeroSeen', scenarioId: '14.5' });

  if (!coverage.d11.driverDoorSeen) missing.d11.push({ signal: 'driverDoorSeen', scenarioId: '14.6' });
  if (!coverage.d11.frontLeftSeen) missing.d11.push({ signal: 'frontLeftSeen', scenarioId: '14.7' });
  if (!coverage.d11.frontRightSeen) missing.d11.push({ signal: 'frontRightSeen', scenarioId: '14.7' });
  if (!coverage.d11.rearLeftSeen) missing.d11.push({ signal: 'rearLeftSeen', scenarioId: '14.7' });
  if (!coverage.d11.rearRightSeen) missing.d11.push({ signal: 'rearRightSeen', scenarioId: '14.7' });
  ...
  if (!coverage.d11.anyDoorOpenSeen) missing.d11.push({ signal: 'anyDoorOpenSeen', scenarioId: '14.6/14.7' });

  if (!coverage.d13.cruiseSetSeen) missing.d13.push({ signal: 'cruiseSetSeen', scenarioId: '14.11' });
  if (!coverage.d13.maxSpeedSeen) missing.d13.push({ signal: 'maxSpeedSeen', scenarioId: '14.12' });
...
export async function runDriveContext(session, opts, out) {
  ...
  if (sampleCount < minSamples) {
    out.fail('Insufficient evidence samples', `captured=${sampleCount}, required=${minSamples}`);
  }
  ...
  d05Covered ? out.pass('D-05 signal activity observed') : out.warn('D-05 signal activity not observed');
  d11Covered ? out.pass('D-11 open-state activity observed') : out.warn('D-11 open-state activity not observed');
  d13Covered ? out.pass('D-13 speed-context activity observed') : out.warn('D-13 speed-context activity not observed');
  ...
  if (report.closureReadiness.d05) out.pass('D-05 closure gate', report.closureReadiness.notes.d05);
  else out.warn('D-05 closure gate', report.closureReadiness.notes.d05);

  if (report.closureReadiness.d11) out.pass('D-11 closure gate', report.closureReadiness.notes.d11);
  else out.warn('D-11 closure gate', report.closureReadiness.notes.d11);

  if (report.closureReadiness.d13) out.pass('D-13 closure gate', report.closureReadiness.notes.d13);
  else out.warn('D-13 closure gate', report.closureReadiness.notes.d13);

  if (report.closureReadiness.allReady) out.pass('Roadmap recommendation', 'D-05/D-11/D-13 can move to done after evidence review');
  else out.warn('Roadmap recommendation', 'Keep tasks in in-review until all closure gates are true');

  if (!report.closureChecklist.ready) {
    out.info(`missing-scenarios: ${report.closureChecklist.missingScenarioIds.join(', ') || 'none'}`);
  }

  if (expectFull && !report.closureReadiness.allReady) {
    out.fail('Expected full closure readiness', 'One or more D-05/D-11/D-13 gates are not satisfied');
  }
