...
describe('assessDriveContextSnapshot', () => {
  ...
  it('flags contradiction when opening is true but anyDoorOpen is false', () => {
    ...
    expect(assessment.errors).toContain('anyDoorOpen=false while one or more openings are true');
  });
...
describe('buildDriveClosureChecklist', () => {
  it('returns empty missing scenarios when all gates are satisfied', () => {
    const coverage = {
      d05: { turnLeftSeen: true, turnRightSeen: true, bsmLeftNonZeroSeen: true, bsmRightNonZeroSeen: true },
      d11: {
        anyDoorOpenSeen: true,
        driverDoorSeen: true,
        frontLeftSeen: true,
      ...
      d13: { cruiseSetSeen: true, accLimitSeen: false, mapLimitSeen: true, maxSpeedSeen: true },
    };
  ...
  it('maps missing signals to expected ESP32-14 scenario IDs', () => {
    const coverage = {
      d05: { turnLeftSeen: false, turnRightSeen: true, bsmLeftNonZeroSeen: false, bsmRightNonZeroSeen: true },
      d11: {
      ...
      d13: { cruiseSetSeen: false, accLimitSeen: false, mapLimitSeen: false, maxSpeedSeen: false },
    };
    const readiness = evaluateDriveClosureReadiness(coverage, true);
    const checklist = buildDriveClosureChecklist(coverage, readiness);

    expect(checklist.ready).toBe(false);
    expect(checklist.missingScenarioIds).toEqual(expect.arrayContaining(['14.1', '14.4', '14.6', '14.7', '14.8', '14.9', '14.11', '14.12', '14.13']));
  });
});
