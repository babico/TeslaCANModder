 ...
 *{box-sizing:border-box;margin:0;padding:0}
 ...
 .snif{font-family:monospace;font-size:9px;background:#0f172a;border:1px solid var(--bor);border-radius:10px;padding:10px;height:100px;overflow-y:auto;line-height:1.8;color:#f59e0b}
 </style>
 </head>
 <body>
 <div class="w">
 ...
 const L={
   tr:{saved:'Ayarlar kaydedildi',stopped:'Tum enjeksiyonlar durduruldu',connErr:'Baglanti hatasi',eeSaved:'EEPROM kaydedildi',canSent:'CAN frame gonderildi',sendErr:'Gonderme hatasi',snifOn:'Sniffer acik',snifOff:'Sniffer kapali'},
   en:{saved:'Settings saved',stopped:'All injections stopped',connErr:'Connection error',eeSaved:'EEPROM saved',canSent:'CAN frame sent',sendErr:'Send error',snifOn:'Sniffer on',snifOff:'Sniffer off'}
 };
 ...
 function fetchStatus(){fetch('/status').then(r=>r.json()).then(d=>{if(!synced){setHW(d.hw!=null?d.hw:1);setP(d.profile!=null?d.profile:2);document.getElementById('ft').checked=!!d.fsd;document.getElementById('ov').checked=!!d.ovr;document.getElementById('ci').checked=d.inj!==false;document.getElementById('ao').value=d.autoOff||0;document.getElementById('flt').value=d.logFilter||0;document.getElementById('snf').checked=!!d.snif;setCrystal(d.crystal!=null?d.crystal:1);document.getElementById('isa').checked=!!d.isa;document.getElementById('ev').checked=d.ev!==false;synced=true;}updateSB(d);syncDriveMode(d);}).catch(()=>{});}
