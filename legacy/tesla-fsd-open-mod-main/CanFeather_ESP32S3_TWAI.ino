...
// Sniffer tamponu
#define SNIF_CAP 16
static CanMsg snifBuf[SNIF_CAP];
...
function fetchStatus(){fetch('/status').then(r=>r.json()).then(d=>{if(!synced){setHW(d.hw!=null?d.hw:1);setP(d.profile!=null?d.profile:2);document.getElementById('ft').checked=!!d.fsd;document.getElementById('ov').checked=!!d.ovr;document.getElementById('ci').checked=d.inj!==false;document.getElementById('ao').value=d.autoOff||0;document.getElementById('flt').value=d.logFilter||0;document.getElementById('snf').checked=!!d.snif;document.getElementById('isa').checked=!!d.isa;document.getElementById('evd').checked=d.ev!==false;synced=true;}updateSB(d);syncDriveMode(d);}).catch(()=>{});}
...
void onConfig() {
  ...
  { char buf[128]; snprintf(buf, sizeof(buf), "[config] hw=%d p=%d fsd=%d ovr=%d inj=%d autoOff=%d filter=%d snif=%d isa=%d ev=%d",
                cfg.hw, cfg.speedProfile, cfg.fsdEnabled, cfg.profileOverride,
                cfg.canInjection, cfg.autoOffMinutes, cfg.logFilter, cfg.snifferMode, cfg.isaSuppress, cfg.emergencyDetect);
    slog(String(buf)); }
...
void onSniff() {
  ...
  for (int i = 0; i < total; i++) {
    CanMsg& f = snifBuf[(start + i) % SNIF_CAP];
...
void onSend() {
  ...
  if (deserializeJson(d, server.arg("plain"))) { server.send(400); return; }
  CanMsg f = {};
  f.id  = (uint16_t)(d["id"] | 0);
  f.dlc = (uint8_t)(d["dlc"] | 8);
  JsonArray data = d["data"];
  for (int i = 0; i < min((int)f.dlc, 8); i++) f.data[i] = (uint8_t)(data[i] | 0);
  canSend(f);
...
void loop() {
  ...
  // Bus-off kurtarma (5 saniyede bir kontrol)
