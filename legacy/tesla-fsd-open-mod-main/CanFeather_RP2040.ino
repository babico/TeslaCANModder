...
void saveConfig() {
  ...
  EEPROM.write(10, cfg.snifferMode ? 1 : 0);
...
void loadConfig() {
  ...
  cfg.snifferMode     = EEPROM.read(10) == 1;
...
void processCommand(const String& cmd, Stream& reply) {
  if      (cmd == "hw0") { cfg.hw = 0; reply.println("[ok] HW=Legacy"); }
  else if (cmd == "hw1") { cfg.hw = 1; reply.println("[ok] HW=HW3"); }
  else if (cmd == "hw2") { cfg.hw = 2; reply.println("[ok] HW=HW4"); }
  ...
  // ── Yeni komutlar: autooff, filter, sniffer, send, save/load, stats ──
    ...
    cfg.snifferMode = true;
    reply.println("[ok] sniffer acik");
  }
  else if (cmd == "snifoff") {
    cfg.snifferMode = false;
    reply.println("[ok] sniffer kapali");
  }
  else if (cmd == "sniflog") {
    ...
    for (int i = 0; i < total; i++) {
      can_frame& sf = snifBuf[(start + i) % SNIF_CAP];
      JsonObject fr = arr.createNestedObject();
      fr["id"]  = sf.can_id;
      fr["dlc"] = sf.can_dlc;
      char hex[24]; snprintf(hex, sizeof(hex), "%02X %02X %02X %02X %02X %02X %02X %02X",
        sf.data[0],sf.data[1],sf.data[2],sf.data[3],sf.data[4],sf.data[5],sf.data[6],sf.data[7]);
      fr["data"] = hex;
    }
...
void setup() {
  ...
  { char buf[100];
    snprintf(buf, sizeof(buf), "[config] hw=%d profile=%d fsd=%d autoOff=%d filter=%d snif=%d xtal=%d",
             cfg.hw, cfg.speedProfile, (int)cfg.fsdEnabled,
             cfg.autoOffMinutes, cfg.logFilter, (int)cfg.snifferMode, cfg.crystal);
    slog(buf);
  }
}
