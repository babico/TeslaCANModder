...
void pushLog(const String& s, uint16_t canId = 0) {
  ...
  logIdx++;
  // NOT: Serial.println burada KALDIRILDI — hot path 100+ fps calisiyor,
  // 115200 baud UART blocking yapiyor ve frame loss penceresini buyutuyor.
  // Loglar hala web UI'dan (logBuf) gorulebilir. slog() (config/hata)
  // hala Serial'e yaziyor cunku nadir cagriliyor.
}
...
// v2.8 fix: SPI bus icin recursive mutex.
// CAN okuma artik Core 0'da ayri FreeRTOS task'inde calisiyor (bkz canReadTask),
// HTTP handler'lar Core 1'de calisiyor. Iki core ayni anda MCP2515'e SPI ile
// erisemez — recursive cunku canReadTask -> processFrame -> canSend zinciri
// ayni mutex'i nested take ediyor.
SemaphoreHandle_t spiMutex = NULL;
#define SPI_LOCK()   do { if (spiMutex) xSemaphoreTakeRecursive(spiMutex, portMAX_DELAY); } while (0)
#define SPI_UNLOCK() do { if (spiMutex) xSemaphoreGiveRecursive(spiMutex); } while (0)

// Forward declaration — canReadTask asagida tanimli ama setup() icinden cagriliyor.
// Arduino'nun otomatik prototip generation'i FreeRTOS callback'lerini yakalayamiyor.
void canReadTask(void *param);
...
<script>
const L={tr:{saved:'Ayarlar kaydedildi',connErr:'Baglanti hatasi',disabled:'Tum enjeksiyonlar durduruldu',eeprom:'EEPROM kaydedildi',canSent:'CAN frame gonderildi',canErr:'Gonderme hatasi',fsdOn:'\u25B6 FSD ACILDI',fsdOff:'\u25A0 FSD KAPANDI',noEvent:'\u2014 henuz olay yok \u2014'},en:{saved:'Settings saved',connErr:'Connection error',disabled:'All injections stopped',eeprom:'EEPROM saved',canSent:'CAN frame sent',canErr:'Send error',fsdOn:'\u25B6 FSD ACTIVATED',fsdOff:'\u25A0 FSD DEACTIVATED',noEvent:'\u2014 no events yet \u2014'}};
...
// CPU'ya gelir. ~500 fps -> ~210 fps, RX FIFO baskisi azalir.
// RXB0: sıkı, en kritik FSD ID'leri
// RXB1: mask trick (alt 3 bit don't care), vehicle control + ISA chime
//   yanlis pozitifler processFrame() icinde reddedilir, fonksiyonel zarar yok.
void applyCanFilters() {
  ...
  // Vehicle control + ISA chime — mask trick
...
void onConfig() {
  ...
  // HW degisirse filter yenile (Legacy ile HW3/HW4 farkli CAN ID'ler)
  if (cfg.hw != prevHw) {
    SPI_LOCK();
    applyCrystal();
    SPI_UNLOCK();
  ...
  { char buf[128]; snprintf(buf, sizeof(buf), "[config] hw=%d p=%d fsd=%d ovr=%d inj=%d autoOff=%d filter=%d crystal=%d isa=%d ev=%d",
                cfg.hw, cfg.speedProfile, cfg.fsdEnabled, cfg.profileOverride,
                cfg.canInjection, cfg.autoOffMinutes, cfg.logFilter, cfg.crystal, cfg.isaSuppress, cfg.emergencyDetect);
    slog(String(buf)); }
...
void setup() {
  ...
  // ESP32'de dual-partition (ota_0/ota_1) var, esp_ota_set_boot_partition ile rollback yapilir
  server.on("/rollback", HTTP_GET, []() {
    const esp_partition_t* running = esp_ota_get_running_partition();
    const esp_partition_t* next = esp_ota_get_next_update_partition(NULL);
    if (!next || next == running) {
      server.send(400, "application/json", "{\"err\":\"Onceki firmware bulunamadi (henuz OTA yapilmamis)\"}");
      return;
    }
    // Diger partition'da gecerli bir app var mi kontrol et
  ...
  // v2.6: Mevcut + diger partition bilgisini ver
  ...
  // v2.8 fix: SPI mutex + Core 0 CAN read task
  // (loop()'tan once yarat, MCP2515 init bittikten sonra)
  spiMutex = xSemaphoreCreateRecursiveMutex();
  if (spiMutex == NULL) {
    Serial.println("[err] spiMutex create failed!");
  } else {
    BaseType_t ok = xTaskCreatePinnedToCore(
      canReadTask,                  // task fonksiyonu
      "canRead",                    // isim
      4096,                         // stack
      NULL,                         // param
      configMAX_PRIORITIES - 2,     // yuksek priority
      NULL,                         // handle
      0                             // CORE 0 (WiFi core)
    );
    if (ok != pdPASS) {
      Serial.println("[err] canReadTask create failed!");
    } else {
      Serial.println("[ok] canReadTask started on Core 0");
    }
  }
...
// ─────────────────────────────────────────────────────────────
//  CAN READ TASK (Core 0) — v2.8 dual-core fix
// ─────────────────────────────────────────────────────────────
//
// v2.7'de loop()'a 'while' eklenmisti ama yetmedi: server.handleClient()
// HTTP istegini islerken (20-60 ms) MCP2515'in 2-frame FIFO'su tasiyor,
// kritik 0x3FD frame'leri kayboluyordu. Sonuc: WiFi acikken FSD "bir gelir
// bir gider" oluyordu.
//
// v2.8 fix: CAN okuma ESP32'nin Core 0'inda (WiFi core) ayri FreeRTOS
// task'inde calisiyor. HTTP/Web Core 1'de (Arduino loopTask) calismaya
// devam ediyor. Iki core paralel, CAN okuma HTTP blocking'inden bagimsiz.
// SPI bus paylasimi spiMutex (recursive) ile sync.
void canReadTask(void *param) {
  can_frame frame;
  for (;;) {
    SPI_LOCK();
    while (mcp && mcp->readMessage(&frame) == MCP2515::ERROR_OK) {
      ...
      // processFrame -> handleHW4 -> canSend zinciri SPI_LOCK altinda
      // (recursive mutex sayesinde nested take guvenli)
      processFrame(frame);
      digitalWrite(PIN_LED, LOW);
    }
    SPI_UNLOCK();
    // 1 ms uyu — bus'tan saniyede ~500 frame geliyor, 1ms tick yeterli
    vTaskDelay(1 / portTICK_PERIOD_MS);
  }
}

// ─────────────────────────────────────────────────────────────
//  LOOP (Core 1 — sadece HTTP + housekeeping)
// ─────────────────────────────────────────────────────────────

void loop() {
  // CAN okuma ARTIK BURADA DEGIL — canReadTask Core 0'da yapiyor.
  // Bu loop sadece WiFi/HTTP ve istatistikler ile ilgileniyor.
  server.handleClient();
  ...
  if (cfg.busAutoRecover && mcp && (now - lastBusCheck > 2000)) {
    lastBusCheck = now;
    SPI_LOCK();
    uint8_t eflg = mcp->getErrorFlags();
    if (eflg & 0x20) {  // BUS-OFF state
      mcp->reset();
      applyCrystal();   // applyCrystal kendi icinde mcp->setBitrate cagiriyor — recursive lock OK
      errCount++;
    }
    SPI_UNLOCK();
    if (eflg & 0x20) {
      slog("[recover] CAN BUS-OFF tespit edildi, MCP2515 reset edildi");
    }
  }
}
