...
/*
 * CanFeather ESP32-S3 TWAI — Tesla FSD CAN Mod + WiFi Arayuzu
 * =============================================================
 * BU KODU YUKLE → Waveshare ESP32-S3-RS485-CAN
 ...
 *
 * BU KARTIN AVANTAJI:
 *   - CAN bus dahili (TWAI) — MCP2515 modulu GEREKMEZ
 *   - WiFi dahili (ESP32-S3) — ayri WiFi modulu GEREKMEZ
 *   - 7-36V guc girisi — Tesla 12V direkt baglanir, USB-C ile de calisir
 *   - DIN rail kutu dahili — proje kutusu GEREKMEZ
 *   - Izole CAN transceiver (TJA1051T/3)
 *
 * DONANIM BAGLANTISI:
 *   Sadece 2 kablo:
 *   Board CAN-H  -->  X179 Pin 13
 *   Board CAN-L  -->  X179 Pin 14
 *
 *   Guc secenekleri:
 *   1) Tesla 12V (X930 Pin 1 veya torpido sigorta) --> Board 7-36V terminal
 *   2) USB-C 5V (test/gelistirme icin)
 *
 * KULLANIM:
 *   1. Kodu yukleyin (USB CDC On Boot: Enabled olmali!)
 *   2. WiFi: "CanFeather" agina baglanin  Sifre: tesla1234
 *   3. Tarayici: http://192.168.4.1
 *
 * GEREKLI KUTUPHANELER:
 *   - ArduinoJson v6 by Blanchon   (Library Manager — v6 secin!)
 *   - WiFi + WebServer (ESP32 board paketi ile gelir)
 *   - driver/twai.h (ESP32 board paketi ile gelir)
 *
 * ONEMLI:
 *   - MCP2515 GEREKMEZ — CAN bus ESP32-S3 TWAI ile dahili
 *   - Flash modu: ESP32S3 Dev Module > USB CDC On Boot > Enabled
 *   - Yuklemek icin: BOOT butonuna basili tut, RESET'e bas-birak, BOOT'u birak
 */

#include <memory>
#include "driver/twai.h"        // ESP32-S3 dahili CAN (TWAI)
#include <WiFi.h>               // ESP32 board paketi
#include <WebServer.h>          // ESP32 board paketi
#include <Update.h>             // ESP32 OTA guncelleme
#include <esp_ota_ops.h>        // v2.6: dual-partition OTA rollback
#include <ArduinoJson.h>        // v6.x — StaticJsonDocument kullaniyor
...
// TWAI (CAN) pin tanimlari — Waveshare ESP32-S3-RS485-CAN
#define CAN_TX_PIN     GPIO_NUM_15
#define CAN_RX_PIN     GPIO_NUM_16

#define PIN_LED        2  // Onboard LED (varsa)
...
// ─────────────────────────────────────────────────────────────
//  CAN FRAME STRUCT (MCP2515 olmadigi icin kendi tanimimiz)
// ─────────────────────────────────────────────────────────────
...
struct Config {
  ...
  bool    canInjection    = true;
  bool    fsdSelectedInUI = false;
  bool    logEnabled      = true;
  uint16_t autoOffMinutes = 0;
  uint16_t logFilter      = 0;
...
void saveConfig() {
  ...
  // byte 8 bos (eski crystal alani, uyumluluk icin)
...
static unsigned long autoOffStart = 0;
...
bool initTWAI() {
  ...
  if (twai_driver_install(&g_config, &t_config, &f_config) != ESP_OK) {
    slog("[can] HATA: TWAI driver kurulamadi!");
    return false;
  }
  if (twai_start() != ESP_OK) {
    slog("[can] HATA: TWAI baslatilamadi!");
    return false;
  }
  twaiReady = true;
  slog("[can] TWAI hazir @ 500 kbps (GPIO15 TX, GPIO16 RX)");
  return true;
}
...
void twaiRecover() {
  ...
  if (twai_get_status_info(&status) == ESP_OK && status.state == TWAI_STATE_BUS_OFF) {
    slog("[can] Bus-off algilandi, kurtariliyor...");
    twai_stop();
...
// ─────────────────────────────────────────────────────────────
//  CAN YARDIMCI FONKSIYONLARI
// ─────────────────────────────────────────────────────────────

inline uint8_t muxID(const CanMsg& f)     { return f.data[0] & 0x07; }
inline bool    fsdInUI(const CanMsg& f)   { return (f.data[4] >> 6) & 0x01; }
inline uint8_t followDist(const CanMsg& f){ return (f.data[5] & 0xE0) >> 5; }

inline void setBit(CanMsg& f, uint8_t bit, bool val) {
  uint8_t mask = 1u << (bit % 8);
  if (val) f.data[bit/8] |=  mask;
  else     f.data[bit/8] &= ~mask;
}

inline void setSpeedV12V13(CanMsg& f, int p) {
  f.data[6] = (f.data[6] & ~0x06) | ((p & 0x03) << 1);
}

void canSend(CanMsg& f) {
  twai_message_t msg = {};
  msg.identifier = f.id;
...
inline void handleNagKiller(CanMsg& f) {
  ...
  // Counter guncelle (AP counter uyusmazligi tespit eder)
  uint8_t cnt = f.data[1] & 0x0F;
  cnt = (cnt + 1) & 0x0F;
  f.data[1] = (f.data[1] & ~0x0F) | cnt;
  // Checksum yeniden hesapla
  uint8_t sum = 0;
...
void processFrame(CanMsg& f) {
  trackFSDState(cfg.fsdSelectedInUI && cfg.fsdEnabled);

  // Arac kontrol frame buffer'larini surekli guncelle
  if (f.id == 0x273) { memcpy(buf_273, f.data, 8); has_273 = true; }
  else if (f.id == 0x2F3) { memcpy(buf_2F3, f.data, 5); has_2F3 = true; }
  else if (f.id == 0x333) { memcpy(buf_333, f.data, 5); has_333 = true; }
  else if (f.id == 0x334) { memcpy(buf_334, f.data, 8); has_334 = true; }
...
void refreshBufs() {
  ...
  while (twai_receive(&rx, 0) == ESP_OK) {
    rxCount++;
    if (rx.identifier == 0x273) { memcpy(buf_273, rx.data, 8); has_273 = true; }
    else if (rx.identifier == 0x2F3) { memcpy(buf_2F3, rx.data, 5); has_2F3 = true; }
    else if (rx.identifier == 0x333) { memcpy(buf_333, rx.data, 5); has_333 = true; }
    else if (rx.identifier == 0x334) { memcpy(buf_334, rx.data, 8); has_334 = true; }
  }
}
...
void execVehicleCmd(const String& cmd) {
  ...
  if (cmd == "climate_keep") {
    ...
    CanMsg cf; memset(&cf,0,sizeof(cf)); cf.id=0x2F3; cf.dlc=5;
  ...
  if (cmd == "charge_start") {
    ...
    CanMsg cf; memset(&cf,0,sizeof(cf)); cf.id=0x333; cf.dlc=5;
...
.dt{font-size:13px;color:var(--mu)}.dt b{color:var(--tx);font-weight:500}
...
.chip{font-family:monospace;font-size:9px;background:#a855f715;border:1px solid #a855f730;color:#c084fc;border-radius:3px;padding:1px 6px;margin-left:6px}
...
<div class="w">
  <div style="margin-bottom:24px;position:relative">
    ...
    <h1><span data-tr="Kontroller" data-en="Controls">Kontroller</span> <span class="badge" id="hwb">HW3</span><span class="chip">ESP32-S3 TWAI</span></h1>
  ...
  <div class="row" style="margin-top:12px;gap:8px">
    ...
    <button class="mbtn" onclick="rollbackFw()" style="background:#f59e0b15;border-color:#f59e0b;color:#f59e0b" title="Onceki firmware'a don">&#8634; Rollback</button>
  </div>
</div>
<div class="toast" id="t"></div>
<script>
const L={tr:{saved:'Ayarlar kaydedildi',connErr:'Baglanti hatasi',disabled:'Tum enjeksiyonlar durduruldu',eeprom:'EEPROM kaydedildi',fsdOn:'\u25B6 FSD ACILDI',fsdOff:'\u25A0 FSD KAPANDI',noEvent:'\u2014 henuz olay yok \u2014'},en:{saved:'Settings saved',connErr:'Connection error',disabled:'All injections stopped',eeprom:'EEPROM saved',fsdOn:'\u25B6 FSD ACTIVATED',fsdOff:'\u25A0 FSD DEACTIVATED',noEvent:'\u2014 no events yet \u2014'}};
let lang=localStorage.getItem('cf_lang')||'tr';
let hw=1,prof=2,synced=false;
...
function rollbackFw(){fetch('/partinfo').then(r=>r.json()).then(d=>{if(!d.canRollback){alert('Onceki firmware bulunamadi. En az 1 OTA yapilmis olmasi lazim.');return;}const v=d.previous&&d.previous.version?d.previous.version:'?';if(!confirm('Onceki firmware\u0027a (v'+v+') donulecek. Cihaz yeniden baslayacak. Devam?'))return;fetch('/rollback').then(r=>r.json()).then(d=>{if(d.ok){toast(d.msg||'Rollback OK');}else{alert('Hata: '+(d.err||'bilinmeyen'));}}).catch(()=>toast(L[lang].connErr));}).catch(()=>alert('Partition bilgisi alinamadi'));}
function save(){fetch('/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({hw,profile:prof,fsd:document.getElementById('ft').checked,ovr:document.getElementById('ov').checked,inj:document.getElementById('ci').checked,autoOff:parseInt(document.getElementById('ao').value)||0,logFilter:parseInt(document.getElementById('flt').value)||0,isa:document.getElementById('isa').checked,ev:document.getElementById('evd').checked,nag:document.getElementById('nag').checked,btlssc:document.getElementById('btlssc').checked,recov:document.getElementById('recov').checked,isaOvr:document.getElementById('isaOvr').checked,isaMul:parseInt(document.getElementById('isaMul').value)||7})}).then(r=>{if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(()=>fetch('/saveee')).then(r=>{if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(()=>{toast(L[lang].saved);document.getElementById('slog').innerHTML='';document.getElementById('log').innerHTML='';synced=false;setTimeout(fetchStatus,500);}).catch(()=>toast(L[lang].connErr));}
...
function fetchStatus(){fetch('/status').then(r=>r.json()).then(d=>{if(!synced){setHW(d.hw!=null?d.hw:1);setP(d.profile!=null?d.profile:2);document.getElementById('ft').checked=!!d.fsd;document.getElementById('ov').checked=!!d.ovr;document.getElementById('ci').checked=d.inj!==false;document.getElementById('ao').value=d.autoOff||0;document.getElementById('flt').value=d.logFilter||0;document.getElementById('isa').checked=!!d.isa;document.getElementById('evd').checked=d.ev!==false;document.getElementById('nag').checked=!!d.nag;document.getElementById('btlssc').checked=!!d.btlssc;document.getElementById('recov').checked=d.recov!==false;document.getElementById('isaOvr').checked=!!d.isaOvr;document.getElementById('isaMul').value=d.isaMul||7;synced=true;}updateSB(d);updateCAN(d);syncDriveMode(d);}).catch(()=>{});}
...
void onStatus() {
  ...
  // CAN bus diagnostik (twai_driver.h:108-127)
  ...
  if (twai_get_status_info(&status) == ESP_OK) {
    ...
    can["state"]    = st;
    can["rx_err"]   = status.rx_error_counter;
    can["tx_err"]   = status.tx_error_counter;
    can["bus_err"]  = status.bus_error_count;
    can["rx_miss"]  = status.rx_missed_count;
  } else {
    can["state"] = "UNKNOWN";
  }
...
void onConfig() {
  ...
  { char buf[128]; snprintf(buf, sizeof(buf), "[config] hw=%d p=%d fsd=%d ovr=%d inj=%d autoOff=%d filter=%d isa=%d ev=%d",
                cfg.hw, cfg.speedProfile, cfg.fsdEnabled, cfg.profileOverride,
                cfg.canInjection, cfg.autoOffMinutes, cfg.logFilter, cfg.isaSuppress, cfg.emergencyDetect);
    slog(String(buf)); }
...
void setup() {
  ...
  delay(1000);
  Serial.begin(115200);
  Serial.println("\n=== CanFeather ESP32-S3 TWAI (Waveshare) ===");
  ...
  // v2.6: OTA Rollback — bir onceki firmware'a don
  server.on("/rollback", HTTP_GET, []() {
    ...
    slog("[rollback] Onceki firmware'a donuluyor...");
    server.send(200, "application/json", "{\"ok\":true,\"msg\":\"Rollback OK, yeniden baslatiliyor...\"}");
    delay(500);
    ESP.restart();
  });
  ...
  // TWAI (CAN) — dahili, SPI yok, MCP2515 yok
  initTWAI();
...
void loop() {
  ...
  // TWAI CAN frame okuma — kuyrugu bosalt
  ...
  // Bus-off kurtarma (her 2 saniyede bir kontrol — runtime toggle)
  static unsigned long lastRecovery = 0;
  if (cfg.busAutoRecover && (now - lastRecovery > 2000)) {
    lastRecovery = now;
    twai_status_info_t status;
    if (twai_get_status_info(&status) == ESP_OK && status.state == TWAI_STATE_BUS_OFF) {
      slog("[recover] CAN BUS-OFF tespit edildi, TWAI reset...");
      twai_stop();
      twai_driver_uninstall();
      initTWAI();
      errCount++;
