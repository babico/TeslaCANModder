...
| ----- | ----------- | --------- | ----------- | ----------- | ------------- |
| Arduino Uno R3 | MCP2515 × 1–3 | BUS_CHASSIS, BUS_VEHICLE, BUS_BODY (flags) | USB Serial, HC-05 BT (optional) | EEPROM | `uno`, `uno_bt` |
...
## UNO-1: First-Time Setup — Chassis Bus Only (uno)
...
| 3 | Open the browser client in Chrome | Client loads, shows "Not Connected" | — |
| 4 | Click "Connect USB" | Browser Web Serial prompt appears | — |
| 5 | Select Arduino COM port | Connection established | — |
| 6 | Verify boot JSON | `{"t":"boot","hw":"ArduinoUnoR3CH340","can":"MCP2515_TJA1050_8MHz","drv":"arduino-mcp2515","busChassis":true,"busVehicle":false,"busBody":false,"bt":"HC-05","btEnabled":false,...}` | — |
...
**Pass Criteria:** Browser client connected, variant defaults to HW4, EEPROM defaults applied (FSD=off, nag=off, profile=0).
...
| 2 | Flash `uno` with `BUS_VEHICLE_ACTIVE=1` | Boot shows `busChassis:true, busVehicle:true` |
| 3 | Bus 0 receives chassis frames | 0x399, 0x3FD, 0x3F8 filtered |
...
| 2 | Flash `uno` with `BUS_VEHICLE_ACTIVE=1 BUS_BODY_ACTIVE=1` | Boot shows `busChassis:true, busVehicle:true, busBody:true` |
...
| 5 | Connect browser client via USB | Same Web Serial flow as Uno | — |
