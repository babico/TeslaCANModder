...
> **Important:** The decoder JSON carries only signal *labels* (enum values), not bit positions or widths. All `SG_` entries use a `0|8@1+` placeholder layout. **`VAL_` enum lines are fully accurate.** You can use the DBC as a starting point and add bit positions manually.
...
| --- | --- | --- | --- |
| `--input <file>` | `-i` | Path to a decoder JSON file | *(required)* |
