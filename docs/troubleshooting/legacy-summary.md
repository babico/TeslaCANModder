...
- **Our project (Tesla-CAN-Mod) is the only one** that combines a multi-handler architecture (HW3/HW4/Legacy), 29+ discrete vehicle features, BLE/WiFi/Serial I/O, native unit tests, and a board-abstracted ESP32 firmware in a single unified codebase.
...
| **Config storage**  | Hardcoded                | EEPROM/NVS                        | NVS on ESP32                        |
| **Firmware update** | Manual flash             | OTA via WiFi                      | OTA on ESP32                        |
...
| **Board-abstracted single codebase**   | ESP32 firmware via per-board files (config/driver/persist/dispatch)                                                                                                                                                                         | Separate codebases per platform                                       |
...
_Generated from automated analysis of 83 legacy repository documentation files. Manual verification recommended for all integration decisions._
