		...
		.object("state",
				[&](JsonLineBuilder::JsonObjectBuilder &o)
				{
					o.boolean("fsd", s.fsdEnabled)
						...
						.boolean("gtwAutopilotSeen", s.gtwAutopilotSeen)
		...
		.object("driverAssist",
				[&](JsonLineBuilder::JsonObjectBuilder &o)
				{
					o.num("turnSignalLeft", s.turnSignalLeft ? 1 : 0)
						...
						.boolean("alcAutoConfirm", s.alcAutoConfirmEnabled)
						.num("dasLaneChangeState", s.dasLaneChangeState)
		...
		.object("vehicle",
				[&](JsonLineBuilder::JsonObjectBuilder &o)
				{
					o.boolean("doorFrontLeftOpen", s.doorFrontLeftOpen)
						...
						.boolean("vehicleLockedState", s.vehicleLockedState)
						.num("regionCode", s.regionCode)
						.num("regionSpoofCode", s.regionSpoofCode)
		...
		.object(
			...
			[&](JsonLineBuilder::JsonObjectBuilder &o)
			{
				o.num("clockReqMHz", s.canClockReqMHz)
					...
					.object("health",
							[&](JsonLineBuilder::JsonObjectBuilder &health)
							{
					...
					.object("frames",
							[&](JsonLineBuilder::JsonObjectBuilder &fr)
							{
								for (uint8_t i = 0; i < BUS_MAX; i++)
									fr.num(kBusName[i], (long)s.canDiag.bus[i].frames);
							})
					.object("hz",
							[&](JsonLineBuilder::JsonObjectBuilder &hz)
							{
								for (uint8_t i = 0; i < BUS_MAX; i++)
									hz.num(kBusName[i], (long)s.canDiag.bus[i].hz);
							})
					.object("hzMin",
							[&](JsonLineBuilder::JsonObjectBuilder &hzMin)
							{
								for (uint8_t i = 0; i < BUS_MAX; i++)
									hzMin.num(kBusName[i],
											  (long)(s.canDiag.bus[i].hzMin == 0xFFFF ? 0 : s.canDiag.bus[i].hzMin));
							})
					.object("hzMax",
							[&](JsonLineBuilder::JsonObjectBuilder &hzMax)
							{
								for (uint8_t i = 0; i < BUS_MAX; i++)
...
void sendStatusCan(State &s)
{
	...
	jsonLine()
		...
		.object("clock", [&](JsonLineBuilder::JsonObjectBuilder &o)
				{ o.num("reqMHz", s.canClockReqMHz).num("activeMHz", s.canClockMHz); })
		.object("health",
				[&](JsonLineBuilder::JsonObjectBuilder &o)
				{
					for (uint8_t i = 0; i < BUS_MAX; i++)
      ...
      o.boolean("wifi", false);
       ...
       .boolean("bodyOnline", false)
		...
		.object("can",
				[&](JsonLineBuilder::JsonObjectBuilder &o)
				{
					o.object("clock", [&](JsonLineBuilder::JsonObjectBuilder &clock)
							 { clock.num("reqMHz", s.canClockReqMHz).num("activeMHz", s.canClockMHz); });
		...
		.object("stream", [&](JsonLineBuilder::JsonObjectBuilder &o)
				{ o.boolean("on", s.streamEnabled).num("emitted", s.streamCount); })
		.boolean("rawCan", s.rawCanListen)
		.end();
}
