...
inline const char* regionName(uint8_t code) {
  switch (code) {
    case REGION_NORTH_AM: return "NA";
    case REGION_EUROPE:   return "EU";
    case REGION_CHINA:    return "CN";
    case REGION_ASIA_PAC: return "APAC";
    case REGION_MIDDLE_E: return "ME";
    default:              return "UNK";
  }
}
...
inline bool applySpoofRegion(Frame& f, uint8_t spoofCode) {
  if (spoofCode == REGION_UNKNOWN) return false;
...
inline bool parseRegionCode(const char* name, uint8_t& out) {
  if (strcmp(name, "na") == 0)   { out = REGION_NORTH_AM; return true; }
  if (strcmp(name, "eu") == 0)   { out = REGION_EUROPE;   return true; }
  if (strcmp(name, "cn") == 0)   { out = REGION_CHINA;    return true; }
  if (strcmp(name, "apac") == 0) { out = REGION_ASIA_PAC; return true; }
  if (strcmp(name, "me") == 0)   { out = REGION_MIDDLE_E; return true; }
  if (strcmp(name, "off") == 0)  { out = REGION_UNKNOWN;  return true; }
  return false;
}

bool executeRegionSpoofCmd(const char* cmd, State& s) {
  if (strncmp(cmd, "region:spoof:", 13) != 0) return false;
  uint8_t code;
  if (!parseRegionCode(cmd + 13, code)) return false;
