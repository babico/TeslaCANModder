...
inline float nagNaturalTorque(float steeringAngleDeg, uint8_t dasHandsOnLevel) {
  ...
  if (steeringAngleDeg > 2.0f || steeringAngleDeg < -2.0f) {
    ...
    if (angleFactor > 0.03f) angleFactor = 0.03f;
    if (angleFactor < -0.03f) angleFactor = -0.03f;
    torque -= angleFactor;
  }

  // Clamp to realistic hand tremor range
  if (torque < 0.08f) torque = 0.08f;
  if (torque > 0.18f) torque = 0.18f;
  return torque;
}
...
inline bool nagKillerShouldEcho(const State& s) {
  if (!s.nagKillerEnabled) return false;
  if (s.nagKillerMode == NAG_KILLER_LEGACY) return true;
  // Both safe and natural modes require DAS feedback
  if (!s.dasSeen) return false;
...
inline bool nagNaturalIntervalReady(State& s, unsigned long nowMs) {
  if (s.nagKillerMode != NAG_KILLER_NATURAL) return false;
  if ((nowMs - s.naturalNagLastMs) < s.naturalNagIntervalMs) return false;
...
bool executeNagCmd(const char* cmd, State& s) {
  if (strncmp(cmd, "nag:", 4) == 0) {
    if (!s.features().nag) return false;
...
bool executeNagKillerCmd(const char* cmd, State& s) {
  ...
  if (strncmp(cmd, "nag:killer:mode:", 16) == 0) {
    if (!s.features().nag) return false;
    NagKillerMode mode;
    if (!parseNagKillerMode(cmd + 16, mode)) return false;
  ...
  if (strncmp(cmd, "nag:killer:", 11) == 0) {
    if (!s.features().nag) return false;
    if (!parseBoolCmd(cmd + 11, s.nagKillerEnabled, s.nagKillerEnabled)) return false;
    resetHandlerLogFlags();
    saveSettings(s);
    return true;
  }
