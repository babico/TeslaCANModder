...
inline void canSimTick(State& s) {
  if (!s.canSimEnabled) return;
  unsigned long now = millis();
  if (now - s.canSimLastMs < 200) return;
  s.canSimLastMs = now;
  s.canSimCounter++;
  ...
  f.data[0] = 0x10; f.data[1] = 0x68; // ~375V
  f.data[2] = 0x00; f.data[3] = 0x32; // ~5A
  handleMessage(f, BUS_VEHICLE, s);

  // Simulate BMS SoC (0x292)
  memset(f.data, 0, 8);
  f.id = CAN_ID_BMS_SOC;
  f.data[0] = 0x02; f.data[1] = 0xBC; // 70%
  handleMessage(f, BUS_VEHICLE, s);
  ...
  if (s.canSimCounter % 5 == 0) {
    ...
    f.data[0] = 250; f.data[1] = 250;
    f.data[2] = 250; f.data[3] = 250;
    // temps ~25°C
    f.data[4] = 25; f.data[5] = 25;
    f.data[6] = 25; f.data[7] = 25;
    handleMessage(f, BUS_VEHICLE, s);
  }
