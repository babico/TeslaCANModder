...
inline void buildPalladiumTurnFrame(Frame& f, bool turnLeft) {
  ...
  f.dlc = 8;
  for (uint8_t i = 0; i < 8; i++) f.data[i] = 0x00;
...
inline bool alcShouldConfirm(const State& s, unsigned long nowMs) {
  if (!s.alcAutoConfirmEnabled) return false;
  if (s.txPaused) return false;
  if (s.dasLaneChangeState != ALC_STATE_REQUESTED) return false;
  if ((nowMs - s.alcLastConfirmMs) < ALC_CONFIRM_COOLDOWN_MS) return false;
  return true;
}
...
inline int8_t alcDirectionFromTurnSignal(const State& s) {
  if (s.turnSignalLeft && !s.turnSignalRight) return -1;
  if (s.turnSignalRight && !s.turnSignalLeft) return 1;
  return 0;
}
...
bool executeAlcCmd(const char* cmd, State& s) {
  if (strncmp(cmd, "alc:", 4) != 0) return false;
