...
bool executeBanShieldCmd(const char* cmd, State& s) {
  if (strncmp(cmd, "banshield:", 10) == 0) {
    if (!parseBoolCmd(cmd + 10, s.banShieldEnabled, s.banShieldEnabled)) return false;
...
bool handleGtwShield(Frame& f, State& s) {
  ...
  if (s.otaInProgress) return false;
  ...
  if (!s.gtwSnapshotValid[mux]) return false;

  bool changed = false;
  for (uint8_t i = 0; i < 8; i++) {
    if (f.data[i] != s.gtwSnapshot[mux][i]) { changed = true; break; }
  }
