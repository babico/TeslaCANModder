...
template<typename Board, typename Transport>
static bool connectToVehicle(Board &board, Transport &client)
{
    char vin[18] = {};
...
static bool runAuthCommand(Board &board, BuildFn &&buildFn, const char *desc)
{
    ...
    if (!connectToVehicle(board, cl)) return false;
...
static void executeTeslaCommand(Board &board, const char *sub)
{
    // ── tesla:key:gen ─────────────────────────────────────────────────────────
    if (strcmp(sub, "key:gen") == 0) {
        KeyPair kp;
        if (!keyGenerate(kp)) { board.err("Tesla: key generation failed"); return; }
        if (!board.saveKey(kp)) { board.err("Tesla: key save failed"); return; }
    ...
    if (strcmp(sub, "key:send") == 0) {
        ...
        auto cl = board.makeTransport();
        if (!connectToVehicle(board, cl)) return;

        uint8_t uuid[16]; board.random(uuid, 16);
        uint8_t reqBuf[200];
        size_t  reqLen = 0;
    ...
    if (strncmp(sub, "charge:amps:", 12) == 0) {
        ...
        if (amps < 1 || amps > 32) { board.err("Tesla: amps must be 1-32"); return; }
        char msg[48]; snprintf(msg, sizeof(msg), "Tesla: charge amps set to %d", amps);
        runAuthCommand(board, [amps](uint8_t *out, size_t cap, size_t &outLen) {
            return buildSetAmpsAction(amps, out, cap, outLen);
        }, msg);
        return;
    }
    ...
    if (strncmp(sub, "charge:limit:", 13) == 0) {
        int pct = atoi(sub + 13);
        if (pct < 50 || pct > 100) { board.err("Tesla: limit must be 50-100"); return; }
        char msg[48]; snprintf(msg, sizeof(msg), "Tesla: charge limit set to %d%%", pct);
        runAuthCommand(board, [pct](uint8_t *out, size_t cap, size_t &outLen) {
            return buildSetLimitAction(pct, out, cap, outLen);
        }, msg);
        return;
    }
    ...
    if (strcmp(sub, "climate:on") == 0) {
        runAuthCommand(board, [](uint8_t *out, size_t cap, size_t &outLen) {
            return buildClimateAction(true, out, cap, outLen);
    ...
    if (strcmp(sub, "climate:off") == 0) {
        runAuthCommand(board, [](uint8_t *out, size_t cap, size_t &outLen) {
            return buildClimateAction(false, out, cap, outLen);
