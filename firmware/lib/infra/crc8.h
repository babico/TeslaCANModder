...
static const uint8_t MAGIC_0x229[16] = {
  0x07, 0x3F, 0x92, 0xE5, 0x4B, 0xD1, 0xA6, 0x78,
  0x23, 0x9C, 0x56, 0xBB, 0xF4, 0x0D, 0xCA, 0x61
};

// CAN 0x249 (DI_steerAssist) — verified
static const uint8_t MAGIC_0x249[16] = {
  0x44, 0xBA, 0x2E, 0x97, 0x51, 0xD3, 0x0F, 0xC8,
  0x76, 0xA1, 0x63, 0x1D, 0xE9, 0x35, 0x8A, 0xFC
};

// CAN 0x370 (EPAS_sysStatus / nag killer) — verified
static const uint8_t MAGIC_0x370[16] = {
  0xE2, 0x7A, 0x14, 0xD8, 0x0B, 0xA3, 0x96, 0x4F,
...
inline uint8_t teslaCrc8(const uint8_t* payload, uint8_t payloadLen,
                          uint8_t counter, const uint8_t* magicTable) {
  // Append a zero byte per Tesla convention
  uint8_t buf[8];
  uint8_t len = payloadLen < 7 ? payloadLen : 7;
  for (uint8_t i = 0; i < len; i++) buf[i] = payload[i];
  buf[len] = 0x00;
