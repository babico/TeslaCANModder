...
// ── Tesla CAN IDs ────────────────────────────────────────────────────────────
#define CAN_ID_LEGACY_STALK 0x045	 // 69 - Legacy stalk position (Model S/X pre-AP, FSD enable trigger)
#define CAN_ID_WINDOW_VENT 0x119	 // 281 - VCLEFT_windowControl (vent open/close/position)
#define CAN_ID_UI_VEHICLE_CTRL 0x273 // 627 - UI_vehicleControl (summon, lock/unlock, park commands)
#define CAN_ID_SENTRY 0x284			 // 644 - UI_sentryMode (sentry mode enable/disable)
#define CAN_ID_CLIMATE 0x2F3		 // 755 - HVAC_request (climate commands: temp, fan speed, mode)
#define CAN_ID_CHARGE 0x333			 // 819 - UI_chargeRequest (charge limit, port open, schedule)
#define CAN_ID_DRIVE_CONFIG 0x334	 // 820 - UI_driveConfig (pedal mode, regen level, creep/roll/hold)
#define CAN_ID_TRUNK_CTRL 0x3B3		 // 947 - UI_trunkControl (frunk/trunk open; glovebox unlatch)
...
#define CAN_ID_BMS_THERMAL 0x312	 // 786 - BMS_thermalStatus (battery pack, coolant, and cell temperatures)
#define CAN_ID_TRACK_MODE 0x313		 // 787 - UI_trackModeSettings (track mode enable, handling profile)
#define CAN_ID_GTW_CAR_STATE                                                                                           \
	0x318 // 792 - GTW_carState (OTA update state: byte[6] bits[1:0]: 0=none, 1=available, 2=installing, 3=scheduled)
...
#define CAN_ID_GTW_CAR_CFG 0x398		 // 920 - GTW_carConfig (HW gen: byte[0] bits[7:6]: 0/1=Legacy, 2=HW3, 3=HW4)
...
#define CAN_ID_DAS_CONTROL 0x2B9		 // 697 - DAS_setSpeed (cruise control set-speed, ACC state)
#define CAN_ID_DAS_STATUS2 0x389		 // 905 - DAS_accSpeedLimit (ACC speed cap from sign/map recognition)
...
#define CAN_ID_TPMS 0x219				 // 537 - VCSEC_TPMSData (tire pressures, all four wheels)
#define CAN_ID_SEATBELT_STATUS 0x3F3	 // 1011 - ID3F3SeatbeltStatus (front/rear buckled flags)
#define CAN_ID_VCFRONT_LIGHTS 0x3F5		 // 1013 - VCFRONT_vehicleLights (exterior light states)
#define CAN_ID_AIR_RECIRC 0x2AA			 // 682 - HVAC_airRecircStatus (recirculation mode)
#define CAN_ID_VEHICLE_SPEED 0x257		 // 599 - ID257VehicleSpeed (signed km/h, ChassisBus)
#define CAN_ID_DI_STATE 0x118			 // 280 - DI_state (gear position, accel pedal, brake)
#define CAN_ID_STEERING_ANGLE 0x129		 // 297 - ID129SteeringAngle (signed 0.1°, ChassisBus)
#define CAN_ID_REAR_MOTOR 0x106			 // 262 - DI_motorRPM (rear motor RPM, all variants)
#define CAN_ID_FRONT_MOTOR 0x115		 // 277 - DI_frontMotorRPM (front motor RPM, dual-motor only)
#define CAN_ID_EPAS_HARNESS 0x229		 // 553 - EPAS_internalHarness (CRC-8 protected)
#define CAN_ID_DI_STEER 0x249			 // 585 - DI_steerAssist (drive mode readback, CRC-8 protected)
#define CAN_ID_VCLEFT_SWITCH 0x3C2		 // 962 - VCLEFT_switchStatus (Palladium turn signal buttons)
#define CAN_ID_ISA_SPEED                                                                                               \
	0x399 // 921 - DAS_ISA speed alert (HW4-only; shares ID 0x399 with BLIND_SPOT — used as HW4 presence discriminator)
#define CAN_ID_LEGACY_FSD_MUX                                                                                          \
	0x3EE // 1006 - Legacy FSD mux (DAS_autopilotControl, Model S/X pre-HW3; presence confirms LEGACY variant)
#define CAN_ID_FOLLOW_DIST 0x3F8 // 1016 - DAS_followDistance (HW3/HW4 follow-distance dial, settings 1–4)
#define CAN_ID_FSD_MUX                                                                                                 \
	0x3FD // 1021 - DAS_FSDControl (HW3/HW4 FSD activation mux; intercepted and modified for FSD inject)
...
#define CAN_ID_FRONT_INV_TEMPS                                                                                         \
	0x376 // 886  - ID376FrontInverterTemps (front inverter/stator/heatsink °C, VehicleBus, dual-motor only)
