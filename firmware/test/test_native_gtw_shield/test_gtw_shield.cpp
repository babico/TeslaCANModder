...
void test_gtw_armed_matching_frame_not_blocked() {
  ...
  s.gtwShieldArmed = true;
  s.gtwSnapshotValid[0] = true;
  for (uint8_t i = 0; i < 8; i++) s.gtwSnapshot[0][i] = (uint8_t)(i * 10);
  Frame f = makeFrame(CAN_ID_GTW_CONFIG_ETH);
  for (uint8_t i = 0; i < 8; i++) f.data[i] = (uint8_t)(i * 10);
  bool result = handleGtwShield(f, s);
...
void test_gtw_armed_changed_frame_restores_snapshot() {
  ...
  s.gtwShieldArmed = true;
  s.gtwSnapshotValid[0] = true;
  for (uint8_t i = 0; i < 8; i++) s.gtwSnapshot[0][i] = 0xAA;
  Frame f = makeFrame(CAN_ID_GTW_CONFIG_ETH);
  for (uint8_t i = 0; i < 8; i++) f.data[i] = 0xAA;
...
void test_gtw_armed_block_counter_increments() {
  ...
  s.gtwShieldArmed = true;
  s.gtwSnapshotValid[0] = true;
  for (uint8_t i = 0; i < 8; i++) s.gtwSnapshot[0][i] = 0x11;
  for (int rep = 0; rep < 3; rep++) {
    Frame f = makeFrame(CAN_ID_GTW_CONFIG_ETH);
    for (uint8_t i = 0; i < 8; i++) f.data[i] = (rep == 0 ? 0x22 : 0x33);
