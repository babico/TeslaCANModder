...
void printStr(const char* s) { capturedOutput += s; }
void printStr(const __FlashStringHelper* s) { capturedOutput += reinterpret_cast<const char*>(s); }
...
void setUp() { capturedOutput.clear(); executedCount = 0; lastExecuted.clear(); }
void tearDown() {}
...
void test_json_line_nested_object() {
  jsonLine().object("inner", [](JsonLineBuilder::JsonObjectBuilder& o) {
    o.str("a", "1").num("b", 2);
  }).end();
...
void test_send_frame_stream_disabled() {
  State s = {}; s.streamEnabled = false;
  Frame f = {}; f.id = 0x123; f.dlc = 2; f.data[0] = 0xAB; f.data[1] = 0xCD;
...
void test_send_frame_stream_enabled_increments_count() {
  State s = {}; s.streamEnabled = true; s.streamCount = 0;
  Frame f = {}; f.id = 0x123; f.dlc = 1; f.data[0] = 0xAA;
  sendFrame(f, "rx", BUS_VEHICLE, 100, s);
...
void test_handleChar_collects_until_newline() {
  State s = {};
  char buf[SERIAL_CMD_BUFFER_SIZE]; uint8_t len = 0;
  const char* cmd = "fsd:on\n";
  for (size_t i = 0; cmd[i]; i++) handleChar(buf, len, cmd[i], s);
...
void test_handleChar_overflow_blocks_execute() {
  ...
  char buf[SERIAL_CMD_BUFFER_SIZE]; uint8_t len = 0;
  // fill past capacity
  for (int i = 0; i < SERIAL_CMD_BUFFER_SIZE + 5; i++) handleChar(buf, len, 'a', s);
  handleChar(buf, len, '\n', s);
