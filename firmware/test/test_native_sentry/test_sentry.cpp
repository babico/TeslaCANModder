/** @file firmware/test/test_native_sentry/test_sentry.cpp
 *  @brief Unit tests for sentry mode commands
...
{
	State s = makeState();
	TEST_ASSERT_TRUE(executeSentryCmd("sentry:off", s));
...
{
	State s = makeState();
	TEST_ASSERT_FALSE(executeSentryCmd("sentry:auto", s));
}
