// ── io/bluetooth/esp32.h Contract Test ─────────────────────────────────────
...
// io/bluetooth/esp32.h ever changes its surface, the parallel implementation
...
struct FakeBT {
  ...
  std::string out;
  std::string inbuf;
  void begin(const char*) { started = true; }
  void print(const char* s) { out += s; }
  void print(const __FlashStringHelper* s) { out += reinterpret_cast<const char*>(s); }
  void print(long n) { char b[32]; snprintf(b, sizeof(b), "%ld", n); out += b; }
  void print(uint8_t b, int base) {
    char buf[4];
    if (base == 16) snprintf(buf, sizeof(buf), "%X", b);
    else            snprintf(buf, sizeof(buf), "%u", b);
    out += buf;
  }
  void println() { out += "\n"; }
...
void btInit() { btSerial.begin("teslamod-test"); btReady = true; }
bool btIsReady() { return btReady; }
void btPrint(const char* s) { if (btReady) btSerial.print(s); }
void btPrintNum(long n) { if (btReady) btSerial.print(n); }
void btPrintHex(uint8_t b) {
  if (!btReady) return;
  if (b < 0x10) btSerial.print("0");
  btSerial.print(b, 16);
}
void btPrintLn() { if (btReady) btSerial.println(); }
int  btAvailable() { return btReady ? btSerial.available() : 0; }
char btRead() { return (char)btSerial.read(); }

void setUp() { btReady = false; btSerial = FakeBT(); }
void tearDown() {}
