...
describe('E2E Smoke: Full Session Lifecycle', () => {
  it('simulates a complete user session', () => {
    const sessionLines = [
      ...
      // 10. Final status
      serial({ t: 'status', up: 10000, rate: 28, fsd: true, sp: 2, banShield: 1, banThreat: 1, banDetectCount: 3 }),
    ];

    const expectedTypes = [
      'boot', 'ack', 'ack', 'status', 'ack',
      'frame', 'frame', 'ack', 'ack', 'ack', 'status',
    ];
...
describe('E2E Smoke: Error Recovery', () => {
  ...
  it('handles firmware error followed by recovery', () => {
    ...
    const statusEv = expectMessage(
      serial({ t: 'status', up: 20000, chassisOnline: true })
    );
