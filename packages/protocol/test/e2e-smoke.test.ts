...
describe('E2E Smoke: Boot Sequence', () => {
  ...
  it('parses a full boot message', () => {
    ...
    expect(boot.busFsd).toBe(true);
    expect(boot.busVehicle).toBe(false);
    expect(boot.fsd).toBe(false);
    expect(boot.canOnline).toBe(true);
  });
...
describe('E2E Smoke: Command → Ack Round-Trip', () => {
  const ackScenarios: Array<{ name: string; cmd: string; ackCmd: string }> = [
    ...
    { name: 'offset -10', cmd: commands.offset(-10), ackCmd: 'offset:-10' },
    { name: 'offset 10', cmd: commands.offset(10), ackCmd: 'offset:10' },
...
describe('E2E Smoke: Full Session Lifecycle', () => {
  it('simulates a complete user session', () => {
    const sessionLines = [
      ...
      // 9. Final status
      serial({ t: 'status', up: 10000, rate: 28, fsd: true, sp: 2 }),
    ];

    const expectedTypes = [
      'boot', 'ack', 'ack', 'status', 'ack',
      'frame', 'frame', 'ack', 'ack', 'status',
    ];
    ...
    const finalStatus = parsed[9].message as any;
...
describe('E2E Smoke: Error Recovery', () => {
  ...
  it('handles firmware error followed by recovery', () => {
    ...
    // Recovery: status shows canOnline again
    const statusEv = expectMessage(
      serial({ t: 'status', up: 20000, canOnline: true })
    );
    expect((statusEv.message as unknown as StatusMessage).canOnline).toBe(true);
  });
});
